...
class mRptLockerPick extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{CALL SP_RPTLockerPick(?,?,?,?,?,?,?,?,?,?,?)}"; 
        $aDataStore = array(
            'pnLngID' => $nLangID, 
    ...
    */
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " 
                ...
                FROM TRPTLockerPickTmp   WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL = "   
            ...
            FROM (
                SELECT
                    ...
                    A.*
                FROM TRPTLockerPickTmp A WITH(NOLOCK)
                /* Calculate Misures */
        ...
        $tSQL .= " ORDER BY L.FDXshDatePick ASC";
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   
            ...
            FROM TRPTLockerPickTmp TSBP WITH(NOLOCK)
            WHERE 1=1
    ...
    public function FSnMCountRowInTemp($paParams)
    {
        ...
        $tSQL = "   
            SELECT
                TMP.FTRsnCode
            FROM TRPTLockerPickTmp TMP WITH(NOLOCK)
