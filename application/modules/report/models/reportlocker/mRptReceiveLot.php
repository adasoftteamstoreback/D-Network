...
class mRptReceiveLot extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{CALL SP_RPTxReceiveLot(?,?,?,?,?,?,?,?,?,?,?)}"; 
        $aDataStore = array(
            'pnLngID'       => $nLangID,
            'ptRptCode'     => $tRptCode,
            'pnComName'     => $tComName,
            ...
            //คลังโอน
            'ptWahT'        => $paDataFilter['tWahCodeOut'],
            'FTResult' => 0
        );
    ...
    */
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " 
                ...
                FROM TRPTReceiveLotTmp   WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL = "   
            SELECT
                L.*,CAST(L.FDXthDocDate as DATE) as FDXthDocDateD
            FROM (
                SELECT
                    ROW_NUMBER() OVER(ORDER BY FTRptCode ASC) AS RowID,
                    A.*
                FROM TRPTReceiveLotTmp A WITH(NOLOCK)
                /* Calculate Misures */
        ...
        $tSQL .= " ORDER BY L.FTPdtBatchID ,L.FTPdtCode ASC";
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   
            ...
            FROM TRPTReceiveLotTmp TSBP WITH(NOLOCK)
            WHERE 1=1
