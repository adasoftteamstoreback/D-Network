...
class mRptRentAmountDetail extends CI_Model {
    ...
    public function FSaMGetDataReport($paDataWhere) {
        ...
        $tSQL = "   SELECT
                            ROW_NUMBER() OVER (PARTITION BY TRPTRTSalDTTmpSplit.FTPosCode ORDER BY TRPTRTSalDTTmpSplit.FTPosCode ASC) AS PARTITIONBYPOS2,
                            TRPTRTSalDTTmpSplit.*
                        FROM (
                                SELECT  
                                    ROW_NUMBER() OVER(ORDER BY FTRptRowSeq) AS RowID,
                                    ROW_NUMBER() OVER (PARTITION BY FTPosCode ORDER BY FTPosCode ASC) AS PARTITIONBYPOS,
                                    COUNT(FTPosCode) OVER (PARTITION BY FTPosCode)  AS COUNTBYPOS, 
                                    *
                                FROM (
                                    SELECT 
                                        FTRptRowSeq,
