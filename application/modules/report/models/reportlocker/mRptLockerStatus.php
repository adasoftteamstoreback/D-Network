...
class mRptLockerStatus extends CI_Model {
    ...
    public function FSnMExecStoreReport($paDataFilter){
        $tCallStore = "{ CALL SP_RPTLockerStatus(?,?,?,?,?,?,?,?,?) }";
        $aDataStore = array(
            ...
            'ptLayStatus'      => $paDataFilter['tLockerStatus'],
            'FNResult'         => 0,
        );
    ...
    public function FSaMGetDataReport($paDataWhere){
       ...
       $tSQL = "SELECT
                    ROW_NUMBER() OVER(PARTITION BY FTBchCode ORDER BY FTBchCode,FTShpCode,FTPosCode,FNLayNo ASC) AS FTBch,
                    SUM(1) OVER(PARTITION BY FTBchCode) AS FTMaxBch,
                    ROW_NUMBER() OVER(PARTITION BY FTBchCode,FTShpCode,FTPosCode ORDER BY FTBchCode,FTShpCode,FTPosCode,FNLayNo ASC) AS FTShp,
                    SUM(1) OVER(PARTITION BY FTBchCode,FTShpCode) AS FTMaxShp,
                    FTBchCode,
                    FTBchName,
                    FTShpCode,
                    FTShpName,
                    ...
                    FTPosName,
                    FNLayNo,
                    CASE WHEN FTLayStaUse = 1 THEN 'ว่าง' 
						WHEN FTLayStaUse = 2 THEN 'ใช้งาน'
						WHEN FTLayStaUse = 3 THEN 'ปิดการใช้งาน'
                        WHEN FTLayStaUse = 4 THEN 'จอง'
						ELSE 'ไม่ระบุ' END AS FTLayStaUse,
                    FTLayName,
                    FTLksRefSale,
                    FTLksRefCstDoc,
                    FTXshRefEXDocNo
                FROM TRPTLockerStatusTmp WITH(NOLOCK)
