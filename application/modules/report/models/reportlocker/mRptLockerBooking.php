...
class mRptLockerBooking extends CI_Model
{
    ...
    public function FSnMExecStoreReport($paDataFilter)
    {
        ...
        $tCallStore = "{CALL SP_RPTLockerBooking(?,?,?,?,?,?,?,?,?,?)}"; 
        $aDataStore = array(
            'pnLngID' => $nLangID, 
    ...
    */
    public function FSaMGetDataReport($paDataWhere)
    {
        ...
        if ($nPage == $nTotalPage) {
            $tRptJoinFooter = " 
                ...
                FROM TRPTLockerBookingTmp   WITH(NOLOCK)
                WHERE 1=1
        ...
        $tSQL = "   
            SELECT
                ROW_NUMBER() OVER (PARTITION BY L.FTBchCode ORDER BY L.FTBchCode ASC) AS PARTITIONBYBCH2,
                ROW_NUMBER() OVER (PARTITION BY L.FTXshDocNo ORDER BY L.FTXshDocNo ASC) AS PARTITIONBYDOC2,
                ROW_NUMBER() OVER (PARTITION BY L.FTXshRefDocNo ORDER BY L.FTXshRefDocNo ASC) AS PARTITIONBYREFDOC2,
                L.*
            FROM (
                SELECT
                    ROW_NUMBER() OVER(ORDER BY FTBchCode ASC) AS RowID,
                    ROW_NUMBER() OVER (PARTITION BY A.FTBchCode ORDER BY A.FTBchCode ASC) AS PARTITIONBYBCH,
                    COUNT(A.FTBchCode) OVER (PARTITION BY A.FTBchCode)  AS COUNTBYBCH, 
                    ROW_NUMBER() OVER (PARTITION BY A.FTXshDocNo ORDER BY A.FTXshDocNo ASC) AS PARTITIONBYDOC,
                    ROW_NUMBER() OVER (PARTITION BY A.FTXshRefDocNo ORDER BY A.FTXshRefDocNo ASC) AS PARTITIONBYREFDOC,
                    A.*
                FROM TRPTLockerBookingTmp A WITH(NOLOCK)
                /* Calculate Misures */
    ...
    private function FMaMRPTPagination($paDataWhere)
    {
        ...
        $tSQL = "   
            SELECT
                COUNT(TSBP.FTXshDocNo) AS rnCountPage
            FROM TRPTLockerBookingTmp TSBP WITH(NOLOCK)
            WHERE 1=1
    ...
    public function FSnMCountRowInTemp($paParams)
    {
        ...
        $tSQL = "   
            SELECT
                TMP.FTXshDocNo
            FROM TRPTLockerBookingTmp TMP WITH(NOLOCK)
