        ...
        $tSQL   =   "   
            ...
            FROM (
                SELECT
                    ...
                    ROW_NUMBER() OVER(PARTITION BY FTPosCode ORDER BY FTPosCode ASC) AS rtPartitionPos ,
                    COUNT(FTPosCode) OVER(PARTITION BY FTPosCode ORDER BY FTPosCode ASC) AS rtPartitionCountPos ,
                    A.*,
                    S.FNRptGroupMember
            ...
            LEFT JOIN (
                SELECT
                    ...
                    FTShpCode AS FTShpCode_SUMPOS,
                    FTPosCode AS FTPosCode_SUMPOS,
                    SUM ( ISNULL( FNXshQTYBill, 0 ) ) AS FNXshQTYBill_SUMPOS,
                    SUM ( ISNULL( FCXshGrand, 0 ) ) AS FCXshGrand_SUMPOS,
                    SUM ( ISNULL( FTXshDepositTime, 0 ) ) AS FTXshDepositTime_SUMPOS 
                FROM
                    TRPTRTDepositAccordingSlotSizeTmp WITH ( NOLOCK ) 
                WHERE
                    1 = 1 
                    AND FTComName = '$tComName' 
                    AND FTRptCode = '$tRptCode' 
            ...
            ) SUMPOS ON L.FTUsrSession = SUMPOS.FTUsrSession_SUMPOS 
            AND L.FTShpCode = SUMPOS.FTShpCode_SUMPOS AND L.FTPosCode = SUMPOS.FTPosCode_SUMPOS
            LEFT JOIN (
            ".$tRptJoinFooter."
        ";
