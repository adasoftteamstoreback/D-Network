...
$("document").ready(function(){
    ...
    if(typeof(nPOStaBrowseType) != 'undefined' && (nPOStaBrowseType == 0 || nPOStaBrowseType==2 )){
        ...
        $("#obtPOSubmitFromDoc").removeAttr("disabled");
        ...
        $('#obtPOCallBackPage').unbind().click(function(){
            ...
            if(typeof(nStaSession) !== "undefined" && nStaSession == 1) {
                ...
                //กดปุ่มย้อนกลับ ถ้ามาจาก "สร้างใบสั่งซื้อผู้จำหน่ายอัตโนมัติ" เวลาย้อนกลับต้องกลับไปหน้าจอเดิม
                if(localStorage.tCheckBackStage == 'PageMangeDocPO'){
        ...
        $('#obtPOApproveDoc').unbind().click(function(){
            ...
            if(tCheckIteminTable>0){
                if(nPOStaValidate==0){
                    if(tFrmSplName!=''){
                        JSxPOSetStatusClickSubmit(2);
                        JSxPOSubmitEventByButton('approve');
                    }else{
        ...
        // JSvPOCallPageList();
        switch(nPOStaBrowseType){
            case '2':
                var tAgnCode = $('#oetPOJumpAgnCode').val();
                var tBchCode = $('#oetPOJumpBchCode').val();
                var tDocNo = $('#oetPOJumpDocNo').val();
                JSvPOCallPageEditDoc(tDocNo);
            break;
            default:
                JSvPOCallPageList();
        }
...
function JSoPOGetAdvanceSearchData(){
    var oAdvanceSearchData  = {
        ...
        tSearchAgnCode      : $("#oetPOAdvSearchAgnCode").val(),
        ...
        tSearchStaRef       : $('#ocmStaDocRef').val()
    };
...
// ReturnType: object
function JSoPODelDocSingle(ptCurrentPage, ptPODocNo,tBchCode,tPORefInCode){
    var nStaSession = JCNxFuncChkSessionExpired();    
    if(typeof nStaSession !== "undefined" && nStaSession == 1) {
...
function JSoPODelDocMultiple(){
    ...
    if (nDataSplitlength > 1) {
        ...
        $('.ocbListItem:checked').each(function(){
                ...
                var tPORefInCode = $(this).data('refincode');
        
                localStorage.StaDeleteArray = '1';
                $.ajax({
                    type: "POST",
                    url: "docPOEventDelete",
                    data: {'tDataDocNo' : tDataDocNo ,'tBchCode':tBchCode,'tPORefInCode':tPORefInCode },
                    cache: false,
                    timeout: 0,
                    success: function (oResult) {
                        var aReturnData = JSON.parse(oResult);
...
function JSxPOValidateFormDocument(){
    ...
    $('#ofmPOFormAdd').validate({
        ...
        messages: {
            ...
            oetPOFrmBchName : {"required" : $('#oetPOFrmBchName').attr('data-validate-required')},
            oetPOToBchName : {"required" : $('#oetPOToBchName').attr('data-validate-required')},
        },
        errorElement: "em",
    ...
    $.ajax({
        ...
        success: function (oResult) {
            ...
            if($("#ohdPOCheckDuplicateCode").val() == 1) {
                //ส่งขอมูลไปรวบรวมที่ Center ก่อนส่ง MQ เพื่อเก็บ LOG (TYPE:ERROR)
                var tLogFunction = 'ERROR';
                var tDisplayEvent = 'เพิ่ม/แก้ไข ใบสั่งซื้อ';
                var tErrorStatus  = '900'
                var tHtmlError = 'Data Duplicate'
                var tLogDocNo   = $('#oetPODocNo').val();
...
// ReturnType: object
function JSxPOSubmitEventByButton(ptType = ''){
    $('#ohdPOApvOrSave').val(ptType);
    if($("#ohdPORoute").val() !=  "docPOEventAdd"){
        var tPODocNo    = $('#oetPODocNo').val();
        var tRefinCode  = $('#oetPORefIntDoc').val();
    }
                ...
                $.ajax({
                    ...
                    success : function(oResult){
                        ...
                        if(aDataReturnEvent['nStaReturn'] == '1'){
                            ...
                            //ส่งขอมูลไปรวบรวมที่ Center ก่อนส่ง MQ เพื่อเก็บ LOG (TYPE:ERROR)
                            var tLogFunction = 'ERROR';
                            var tDisplayEvent = 'บันทึก/แก้ไข ใบสั่งซื้อ';
                            var tErrorStatus = '500';
                            var tMsgErrorBody = tMessageError;
                            var tLogDocNo   = tPODocNo;
                            JCNxPackDataToMQLog(tMsgErrorBody,tErrorStatus,tDisplayEvent,tLogFunction,tLogDocNo);
                        }
                    },
                    error   : function (jqXHR, textStatus, errorThrown) {
...
//         var tPODocNo    = $('#oetPODocNo').val();
//     }
//      $('.xWConditionSearchPdt').each(function (e) { 
//         $( this ).prop( "disabled", false );
//     });
//     $( "#ocbPOFrmInfoOthStaDocAct" ).prop( "disabled", false );
    ...
    if(typeof nStaSession !== "undefined" && nStaSession == 1) {
        ...
        $.ajax({
            ...
            error: function (jqXHR, textStatus, errorThrown){
                ...
                if (jqXHR.status != 404){
                    var tLogFunction = 'ERROR';
                    var tDisplayEvent = 'เรียกดูเอกสารใบสั่งซื้อ';
                    var tErrorStatus  = jqXHR.status;
                    var tLogDocNo   = ptPODocNo;
                    var tHtmlError = $(jqXHR.responseText);
                    var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
...
function JCNxPOControlObjAndBtn(){
    ...
    //control ฟอร์ม
    if(nPOStaDoc == 3 || (nPOStaDoc == 1 && nPOStaApv == 1)){ //เอกสารยกเลิก
        //ปุ่มเลือก
...
function JSnPOCancelDocument(pbIsConfirm){
    var tPODocNo    = $("#oetPODocNo").val();
    var tPORefIntDoc        = $("#oetPORefIntDoc").val();
    if(pbIsConfirm){
        $.ajax({
            type: "POST",
            url: "docPOCancelDocument",
            data: {'ptPODocNo' : tPODocNo,'tPORefIntDoc' : tPORefIntDoc},
            cache: false,
            timeout: 0,
            success: function (tResult) {
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                if (jqXHR.status != 404){
                    var tLogFunction = 'ERROR';
                    var tDisplayEvent = 'ยกเลิกใบสั่งซื้อ';
                    var tErrorStatus = jqXHR.status;
                    var tHtmlError = $(jqXHR.responseText);
                    var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                    var tLogDocNo = tPODocNo;
        ...
        $.ajax({
            ...
            success: function(tResult){
                ...
                if (oResult.nStaEvent == "900") {
                    ...
                    var tDisplayEvent = 'อนุมัติใบสั่งซื้อ';
                    var tErrorStatus = '';
