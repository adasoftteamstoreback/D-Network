...
$("document").ready(function() {
    ...
    if (typeof(nStaASTBrowseType) != 'undefined' && (nStaASTBrowseType == 0 || nStaASTBrowseType == 2)) { // เข้ามาจาก Menulist Tab
        ...
        switch (nStaASTBrowseType) {
            case '2':
                JSxASTNavDefult();
                var tDocNo = $('#oetASTJumpDocNo').val();
                JSvASTCallPageEdit(tDocNo);
                break;
            default:
                JSxASTNavDefult();
                JSvASTCallPageList();
...
function JSvASTCallPageEdit(ptXthDocNo) {
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            if (jqXHR.status != 404) {
                var tLogFunction = 'ERROR';
                var tDisplayEvent = 'เรียกดูเอกสารใบตรวจนับ';
                var tErrorStatus = ''
                var tLogDocNo = ptXthDocNo;
                var tHtmlError = $(jqXHR.responseText);
                var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                JCNxPackDataToMQLog(tMsgErrorBody, tErrorStatus, tDisplayEvent, tLogFunction, tLogDocNo);
            } else {
                //JCNxSendMQPageNotFound(jqXHR,ptXthDocNo);
            }
        }
    });
}
...
function JSvASTLoadPdtDataTableHtml(pnPage) {
    ...
    var tASTAjhStaStkApv = $("#ohdASTAjhStaStkApv").val();
    ...
    $.ajax({
        ...
        data: {
            ...
            'ptASTAjhStaStkApv': tASTAjhStaStkApv,
        ...
        success: function(tResult) {
            ...
            $('#ofmASTFormAdd').validate({
                ...
                invalidHandler: function(event, validator) {
                    //ส่งขอมูลไปรวบรวมที่ Center ก่อนส่ง MQ เพื่อเก็บ LOG (TYPE:ERROR)
                    var tLogFunction = 'ERROR';
                    var tDisplayEvent = 'เพิ่ม/แก้ไข ใบตรวจนับ';
                    var tErrorStatus = '900'
                    var tHtmlError = 'Data Duplicate'
                    var tLogDocNo = $("#oetASTDocNo").val();
...
function JSxSubmitEventByButton() {
    ...
    $("#oetASTCountType").prop("disabled", false);
            ...
            if (bReturn) {
                $.ajax({
                        ...
                        if (nStaASTBrowseType != 1) {
                            ...
                            if (aReturn["nStaEvent"] == 1) {
                                var oSOCallDataTableFile = {
                                        ...
                                        ptBchCode: $("#ohdASTBchCode").val(), //รหัสสาขา
                                        ptDocNo: aReturn['tCodeReturn'], //เลขที่เอกสาร
                                        ptDocKey: 'TCNTPdtAdjStkHD', //ชื่อตารางของเอกสาร
                                    }
                                    //console.log(oSOCallDataTableFile);
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        if (jqXHR.status != 404) {
                            var tLogFunction = 'ERROR';
                            var tDisplayEvent = 'บันทึก/แก้ไข ใบตรวจนับ';
                            var tErrorStatus = jqXHR.status;
                            var tHtmlError = $(jqXHR.responseText);
                            var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                            var tLogDocNo = tDocNo;
...
function JCNxASTControlObjAndBtn() {
    ...
    var tType = $("#ohdASTUserType").val();
    var tCountType = $("#ohdASTAjhCountType").val();
    var tStaStkApv = $("#ohdASTAjhStaStkApv").val();
    ...
    $("#obtASTHQApprove").attr("disabled", false);
    ...
    if (ohdXthStaApv == 1) {
        ...
        //เหตุผลในการยกเลิก
        $('#obtASTBrowseRsnCancel').attr("disabled", false);
        $('#oetASTRsnNameCancel').attr("disabled", false);
    ...
    if (ohdASTStaDoc == 3) {
        ...
        //เหตุผลในการยกเลิก
        $('#obtASTBrowseRsnCancel').attr("disabled", true);
        $('#oetASTRsnNameCancel').attr("disabled", true);
...
function JSnASTCancelDoc(pbIsConfirm) {
    ...
    if (pbIsConfirm) {
        ...
        //บังคับให้เลือกเหตุผลในการยกเลิก
        if ($('#oetASTRsnCodeCancel').val() == '' || $('#oetASTRsnCodeCancel').val() == null) {
            $('#oetASTRsnNameCancel').focus();
            return;
        }
        ...
        $.ajax({
            ...
            data: {
                tXthDocNo: tXthDocNo,
                tReasonCancel: $("#oetASTRsnCodeCancel").val()
            },
            cache: false,
            timeout: 5000,
            success: function(tResult) {
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                ...
                if (jqXHR.status != 404) {
                    var tLogFunction = 'ERROR';
                    var tDisplayEvent = 'ยกเลิกใบตราจนับ';
                    var tErrorStatus = jqXHR.status;
                    var tHtmlError = $(jqXHR.responseText);
                    var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                    ...
                    //JCNxSendMQPageNotFound(jqXHR,tXthDocNo);
                }
            }
        });
    } else {
...
function JSnASTApprove(pbIsConfirm) {
    ...
    if (typeof nStaSession !== "undefined" && nStaSession == 1) {
        try {
            if (pbIsConfirm) {
                // JSxASTBCHCheckDocApvAllInDB().then(function(res) {
                // var oResult = JSON.parse(res);
                // if(oResult.nCode == 400){
                //     return;
                // }else{
...
//สำนักงานใหญ่อนุมัติ
function JSnASTHQApprove(pbIsConfirm) {
    var nStaSession = JCNxFuncChkSessionExpired();
    if (typeof nStaSession !== "undefined" && nStaSession == 1) {
        try {
            if (pbIsConfirm) {
                $('#odvASTModalHQAppoveDoc').modal("hide");
                var tXthDocNo = $("#oetASTDocNo").val();
                var tXthStaApv = $("#ohdASTStaApv").val();
                var tASTBchCode = $("#oetASTBchCode").val();
                var tASTCountType = $("#oetASTCountType").val();
                $.ajax({
                    type: "POST",
                    url: "dcmASTHQApprove",
                    data: {
                    ...
                    success: function(tResult) {
                        ...
                        if (oResult["nStaEvent"] == "900") {
                            ...
                            var tDisplayEvent = 'อนุมัติใบตรวจนับ (HQ)';
                            var tErrorStatus = '';
                ...
                $('#odvASTModalHQAppoveDoc').modal("show");
            }
        } catch (err) {
            console.log("JSnTFWApprove Error: ", err);
        }
...
function JSxASTBCHAprroveDoc() {
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            if (jqXHR.status != 404) {
                var tLogFunction = 'ERROR';
                var tDisplayEvent = 'อนุมัติใบตรวจนับ';
                var tErrorStatus = jqXHR.status;
                var tHtmlError = $(jqXHR.responseText);
                var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                var tLogDocNo = tXthDocNo;
...
function JSxASTBCHCheckDocApvAllInDB() {

    return $.ajax({
        type: "POST",
        url: "dcmASTCheckDocAllAproveINBCH",
        data: {
        ...
        success: function(tResult) {
            ...
            if (oResult.nCode == 400) {
                $('#odvASTModalDocNotApv').modal('show');
            }
        },
        error: function(jqXHR, textStatus, errorThrown) {
            JCNxResponseError(jqXHR, textStatus, errorThrown);
...
function JSxASTHQSendNotiForDocumentNotApv() {
    $.ajax({
        type: "POST",
        url: "dcmASTSendNotiForDocNotApv",
        data: {
            ptBCHCode: $("#oetASTBchCode").val()
        },
        cache: false,
...
function JSoASTSubscribeMQ() {
    ...
    var tUsrApv = $("#ohdASTProveUser").val();
    var tDocNo = $("#oetASTDocNo").val();
    var tPrefix = 'RESAJS';
...
function JSxASTEventAddProductsByBarCode(ptPDTCode, ptBarCode) {
    ...
    $.ajax({
        type: "POST",
        url: "docAdjStkEventAddProductsByBarCode",
        data: {
            ptPDTCode: ptPDTCode,
            ptBarCode: ptBarCode,
            paDataInsert: aDataInsert
