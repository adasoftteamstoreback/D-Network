...
class arrangementproduct_controller extends MX_Controller {
    ...
    public function FSoCPAMPageHDDocRefList(){
        try{
            ...
            $aDataDocHDSORef = $this->arrangementproduct_model->FSaMPAMGetDataHDSORefTmp($aDataWhere);
            if(isset($aDataDocHDSORef['aItems']['FTXthRefDocNo'])){
                $Refdocno = $aDataDocHDSORef['aItems']['FTXthRefDocNo'];
            }else{
                $Refdocno = '';
            }
            $aDataStaBooking = $this->arrangementproduct_model->FSaMPAMGetDataStaBooking($tDocNo, $Refdocno);
            ...
            $aReturnData = array(
                ...
                'nStaEvent'         => '1',
                'tStaMessg'         => 'Success',
                'aDataStaBooking'   => $aDataStaBooking,
                'tSODocRef'         => $Refdocno
            );
        } catch (Exception $Error) {
            $aReturnData = array(
    ...
    public function FSoCPAMApproveEvent(){
        try{
            ...
            $tPCKDocNo = $this->input->post('tDocNo');
            $tPCKBchCode = $this->input->post('tBchCode');
            /// Check Stock
            $aChkStock = $this->arrangementproduct_model->FSaMPCKCheckStk($tPCKDocNo);
            if($aChkStock['nStaEvent'] == 1){
                $this->db->trans_begin();
                ...
                $aStatus = $this->arrangementproduct_model->FSaMPCKStaprcDocument($aDataUpdate);

                $aReturnData     = array(
                    'nStaEvent'    => '4',
                    'tStaMessg'    => 'สินค้าบางรายการสต็อคไม่เพียงพอ',
                );
            }
        } catch (Exception $Error) {
            $aReturnData = array(
