<script type="text/javascript">
    ...
    $(document).ready(function(){
        var nStaDocRef = $("#ohdTWOnStaDocRef").val();

        if (nStaDocRef==3 || nStaDocRef =="") {
          //$("#obtTRBCancelDoc").show();
          $("#obtTRBCancelDoc").toggle(true);
        }
        ...
        $("#obtTRBSubmitFromDoc").removeAttr("disabled");
    ...
    var oWahOption      = function(poDataFnc){
        ...
        if($('#ohdTWOnStaWasteWAH').val() == ""){
            var tFindWahouse = " AND TCNMWaHouse.FTWahStaType IN('1','2','5') ";
        }else{
            var tFindWahouse = " AND TCNMWaHouse.FTWahStaType IN('1','2','5','10') ";
        }
        ...
        var oOptionReturn   = {
            ...
            Where: {
                Condition : [" AND TCNMWaHouse.FTBchCode='"+tTRBBchCode+"' "+tFindWahouse +" "]
            },
            GrideView:{
    ...
    // ตัวแปร Option Browse Modal สาขาต้นทาง
    var oBranchOption = function(poDataFnc){
        ...
        var oOptionReturn       = {
            ...
            GrideView: {
                ...
                ColumnKeyLang       : ['tBrowseBCHCode', 'tBrowseBCHName', '', '', 'tUSRAgency'],
                ColumnsSize         : ['20%','40%','','','40%'],
                DataColumns         : ['TCNMBranch.FTBchCode', 'TCNMBranch_L.FTBchName','TCNMWaHouse_L.FTWahCode','TCNMWaHouse_L.FTWahName','TCNMAgency_L.FTAgnName'],
                DataColumnsFormat   : ['', ''],
                DisabledColumns     : [2,3],
                WidthModal          : 50,
                Perpage             : 10,
    ...
    // ตัวแปร Option Browse Modal สาขาปลายทาง
    ...
    function JSxNextFuncDOBchTo() {
        ...
        if($('#oetTRBFrmBchCodeTo').val() != ''){
            $.ajax({
                type    : "POST",
                url     : "docTransferBchOutCheckWahouseInBCH",
                data    : {
                    tBCHCode    : $('#oetTRBFrmBchCodeTo').val(),
                },
                cache   : false,
                timeout : 5000,
                success : function(oResult) {
                    var aResult = JSON.parse(oResult)
                    $('#oetTRBFrmWahCodeTo').val(aResult.aItems[0].FTWahCode);
                    $('#oetTRBFrmWahNameTo').val(aResult.aItems[0].FTWahName);
                },
    ...
    function JSxNextFuncDOBchShip() {
        ...
        //หาว่า สาขานี้คลัง default คือคลังอะไร
        if($('#oetTRBFrmBchCodeShip').val() != ''){
            $.ajax({
                type    : "POST",
                url     : "docTransferBchOutCheckWahouseInBCH",
                data    : {
                    tBCHCode    : $('#oetTRBFrmBchCodeShip').val(),
                },
                cache   : false,
                timeout : 5000,
                success : function(oResult) {
                    var aResult = JSON.parse(oResult)
                    $('#oetTRBFrmWahCodeShip').val(aResult.aItems[0].FTWahCode);
                    $('#oetTRBFrmWahNameShip').val(aResult.aItems[0].FTWahName);
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    JCNxCloseLoading();
                    JCNxResponseError(jqXHR, textStatus, errorThrown);
    ...
    // ========================================== Brows Event Conditon ===========================================
        ...
        function JSxCallTRBRefIntDoc(){
            // var tBCHCode = $('#oetTRBFrmBchCode').val();
            // var tBCHName = $('#oetTRBFrmBchName').val();
            var tBCHCode = '<?=$this->session->userdata('tSesUsrBchCodeDefault');?>';
            var tBCHName = '<?=$this->session->userdata('tSesUsrBchNameDefault');?>';
...
//------------------------------------------------------------------------------------------------//
    ...
    function JSxTRBSubmitEventByButton(ptType = ''){
        var tTRBDocNo = '';

        if($("#ohdTRBRoute").val() !=  "docTRBEventAdd"){
            var tTRBDocNo    = $('#oetTRBDocNo').val();
        }
        $('#obtTRBSubmitFromDoc').attr('disabled', true);
