<script>
            ...
            $('#obtTRBApproveDoc').unbind().click(function() {
                ...
                if (typeof(nStaSession) !== "undefined" && nStaSession == 1) {
                    ...
                    if (tCheckIteminTable > 0) {
                        ...
                        JSxTRBSubmitEventByButton('approve')
    ...
    function JSxTRBControlFormWhenCancelOrApprove() {
        let tStatusDoc      = $('#ohdTRBStaDoc').val();
        let tStatusApv      = $('#ohdTRBStaApv').val();
        let tStatusRef      = $('#ohdTRBStaRefKey').val();
        let tAutStaCancel   = $('#ohdTRBAutStaCancel').val();
        let tDocDateCreate  = $('#ohdTRBDocDateCreate').val();
        let tDocDateToday   = $('#ohdTRBDocDateToday').val();
        ...
        if (tStatusDoc == 3) {
            ...
            if(tDocDateCreate != "" && tDocDateToday != "" && DocDateCreate != tDocDateToday){
                if(tAutStaCancel == '1'){
                    $('#obtTRBCancelDoc').show();
                }else{
                    $('#obtTRBCancelDoc').hide();
        ...
        // ถ้าสาขาต้นทาง = สาขาที่ user ใช้งานจะโชว์ปุ่มให้ไปจ่ายโอน
        if ($('#oetTRBFrmBchCodeTo').val() == '<?= $this->session->userdata("tSesUsrBchCodeDefault") ?>') {
            $("#obtTRBJumpToPageTBO").show();
        }
    }
    ...
    //ไปเอกสารจ่ายโอน
    function JSxTRBJumpToPageTBO() {
        var tRoute = 'docTransferBchOut/0/0';
        var tDocumentNumber = $('#oetTRBDocNo').val();
        var tWahouseTo = $('#oetTRBFrmWahCodeShip').val();
        var tWahouseNameTo = $('#oetTRBFrmWahNameShip').val();
        var tBranchTo = $('#oetTRBFrmBchCodeShip').val();
        var tBranchNameTo = $('#oetTRBFrmBchNameShip').val();
        ...
        $.ajax({
            ...
            success: function(tResult) {
                ...
                setTimeout(function() {
                    JSvTransferBchOutCallPageAdd_JumpTRB(tDocumentNumber, tWahouseTo, tWahouseNameTo, tBranchTo, tBranchNameTo);
                }, 2000);
            },
            error: function(jqXHR, textStatus, errorThrown) {
                JCNxResponseError(jqXHR, textStatus, errorThrown);
