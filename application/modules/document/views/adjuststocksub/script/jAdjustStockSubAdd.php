...
function JSxAdjStkSubOpenImportForm(){
        if($('#oetAdjStkSubWahCodeTo').val()!=""){
            var tNameModule     = 'adjstock';
            var tTypeModule     = 'document';
            var tAfterRoute     = 'JSxAdjStkSubAfterImportForm'; // call func
            var tFlagClearTmp   = '1' // null = ไม่สนใจ 1 = ลบหมดเเล้วเพิ่มใหม่ 2 = เพิ่มต่อเนื่อง
            var tDocumentNo = $("#oetAdjStkSubAjhDocNo").val();
            var tPIFrmBchCode = $("#ohdAdjStkSubBchCodeTo").val();
            var nSplVatRate = '';
            var tSplVatCode = '';
            var aPackdata = {
...
function JSxAdjStkSubAfterImportForm(paData){
   
    var oPackData = JSON.parse(paData);
    
    // console.log(oPackData['aPdtCallBackFunction']);
    JSvAdjStkSubLoadPdtDataTableHtml(1);
    if(oPackData['aPdtCallBackFunction'].length>0){
        var oOptionForFashion = {
                    'bListItemAll'   : true,
                    'tSpcControl'  : '',
                    'tNextFunc' : 'JSxAdjStkSubInsertDTFhnToTmp'
                }
        JSxCheckProductSerialandFashion(oPackData['aPdtCallBackFunction'],oOptionForFashion,'insert');
    }
