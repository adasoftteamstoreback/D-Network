<script type="text/javascript">
    ...
    var tUserBchCode        = '<?= $this->session->userdata("tSesUsrBchCodeDefault"); ?>';
    var tUserBchName        = '<?= $this->session->userdata("tSesUsrBchNameDefault"); ?>';
    var tUserWahCode        = '<?= $this->session->userdata("tSesUsrWahCode"); ?>';
    var tUserWahName        = '<?= $this->session->userdata("tSesUsrWahName"); ?>';
    var tUserBchCodeto      = '<?= $this->session->userdata("tSesUsrBchCodeDefault"); ?>';
    var tUserBchNameto      = '<?= $this->session->userdata("tSesUsrBchNameDefault"); ?>';
    var tUserWahCodeto      = '<?= $this->session->userdata("tSesUsrWahCode"); ?>';
    var tUserWahNameto      = '<?= $this->session->userdata("tSesUsrWahName"); ?>';
    ...
    var tStaDoc             = '<?= $tTBIStaDoc; ?>';
    var tStaApv             = '<?= $tTBIStaApv; ?>';
    var tRoute              = '<?= $tTBIRoute; ?>';
    var tStaPrcStk          = '<?= $tTBIStaPrcStk; ?>';
    ...
    var oBrowseTBIBPdtIntBranchBrows = {
        ...
        Join: {
            Table: ['TCNMBranch_L', 'TCNMWaHouse_L', 'TCNTPdtTboHDRef'],
            On: [
                ...
                'TCNTPdtIntDTBch.FTXthDocNo = TCNTPdtTboHDRef.FTXthDocNo'
            ]
        },
        Where: {
            Condition: []
        ...
        GrideView: {
            ...
            ColumnKeyLang: ['tTBIBchCode', 'จ่ายโอนจากสาขา', 'tTBIBrowsDocDate', 'tTBIBrowsDocNo', 'tTBIWarehouseTo'],
            ColumnsSize: ['10%', '20%', '20%', '30%', '20%'],
            WidthModal: 50,
            DataColumns: ['TCNTPdtIntDTBch.FTBchCode', 'TCNMBranch_L.FTBchName', 'TCNTPdtIntDTBch.FDCreateOn', 'TCNTPdtIntDTBch.FTXthDocNo', 'TCNMWaHouse_L.FTWahName', 'TCNTPdtIntDTBch.FTXthWahTo', 'TCNTPdtTboHDRef.FTXthCtrName', 'TCNTPdtTboHDRef.FDXthTnfDate', 'TCNTPdtTboHDRef.FTXthRefTnfID', 'TCNTPdtTboHDRef.FTXthRefVehID', 'TCNTPdtTboHDRef.FTXthQtyAndTypeUnit'],
            DataColumnsFormat: ['', '', '', '', ''],
            DisabledColumns: [5, 6, 7, 8, 9, 10],
            Perpage: 10,
        ...
        NextFunc: {
            ...
            ArgReturn: ['FTXthDocNo', 'FTBchCode', 'FTBchName', 'FTWahName', 'FTXthWahTo', 'FTXthCtrName', 'FDXthTnfDate', 'FTXthRefTnfID', 'FTXthRefVehID', 'FTXthQtyAndTypeUnit'],
        },
        //  DebugSQL : true
    }
    ...
    function JSxTBISelectDocRefer(ptCode) {
        if (ptCode == 'NULL' || ptCode == null) {
            ...
            //การขนส่ง
            let tTBOCtrName = tResult[5];
            let tTBITnfDate = tResult[6];
            let tTBIRefTnfID = tResult[7];
            let tTBIRefVehID = tResult[8];
            let tTBIQtyAndTypeUnit = tResult[9];
            ...
            $('#oetTBITransportCtrName').val(tTBOCtrName);
            $('#oetTBITransportTnfDate').datepicker("setDate", tTBITnfDate);
            $('#oetTBITransportRefTnfID').val(tTBIRefTnfID);
            $('#oetTBITransportRefVehID').val(tTBIRefVehID);
            $('#oetTBITransportQtyAndTypeUnit').val(tTBIQtyAndTypeUnit);
    ...
    $('#obtTBIBrowseWahTo').click(function() {
        ...
        oTBIBrowseWahTo.Where.Condition = ["AND TCNMWaHouse.FTBchCode = '" + tBCH + "' " + tFindWahouse + " "];
    ...
    function JSxTransferreceiptEventAddEdit(ptRoute) {
        $("#ofmTransferreceiptFormAdd").validate({
            ...
            submitHandler: function(form) {
                $('#obtTBISubmitFromDoc').attr('disabled', true);

                if ($('#ohdTBIRoute').val() == 'docTBIEventAdd') {
                    if (!$('#ocbTBIStaAutoGenCode').is(':checked')) {
                        JSxTBIValidateDocCodeDublicate();
                    } else {
                        JSxTBISubmitEventByButton(ptRoute);
                    }
                } else {
                    JSxTBISubmitEventByButton(ptRoute);
                }
    ...
    function JSxTBIValidateDocCodeDublicate() {
        $.ajax({
            type: "POST",
            url: "CheckInputGenCode",
            data: {
                'tTableName': 'TCNTPdtTbiHD',
                'tFieldName': 'FTXthDocNo',
                'tCode': $('#oetTBIDocNo').val()
            },
            success: function(oResult) {
                var aResultData = JSON.parse(oResult);
                $("#ohdTBICheckDuplicateCode").val(aResultData["rtCode"]);

                if ($("#ohdTBICheckClearValidate").val() != 1) {
                    $('#ofmTransferreceiptFormAdd').validate().destroy();
                }
                ...
                $.validator.addMethod('dublicateCode', function(value, element) {
                    if ($("#ohdTBIRoute").val() == "docTBIEventAdd") {
                        if ($('#ocbTBIStaAutoGenCode').is(':checked')) {
                            ...
                            if ($("#ohdTBICheckDuplicateCode").val() == 1) {
                                return false;
                            } else {
                                return true;
                            }
                ...
                $('#ofmTransferreceiptFormAdd').validate({
                    ...
                    messages: {
                        oetTBIDocNo: {
                            "dublicateCode": $('#oetTBIDocNo').attr('data-validate-duplicate')
                        }
                    },
                    errorElement: "em",
                    errorPlacement: function(error, element) {
                    ...
                    submitHandler: function(form) {
                        JSxTBISubmitEventByButton($('#ohdTBIRoute').val());
                    }
                })

                if ($("#ohdTBICheckClearValidate").val() != 1) {
                    $("#ofmTransferreceiptFormAdd").submit();
                    $("#ohdTBICheckClearValidate").val(1);
                }
                ...
                if ($("#ohdTBICheckDuplicateCode").val() == 1) {
                    ...
                    var tDisplayEvent = 'เพิ่ม/แก้ไข ใบรับโอน - สาขา';
                    var tErrorStatus = '900'
                    var tHtmlError = 'Data Duplicate'
                    var tLogDocNo = $('#oetTBIDocNo').val();
        ...
        if (tItem > 0) {
            ...
            $.ajax({
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    if (jqXHR.status != 404) {
                        var tLogFunction = 'ERROR';
                        var tDisplayEvent = 'บันทึก/แก้ไข ใบรับโอน - สาขา';
                        var tErrorStatus = jqXHR.status;
                        var tHtmlError = $(jqXHR.responseText);
                        var tMsgErrorBody = tHtmlError.find('p:nth-child(3)').text();
                        var tLogDocNo = tDocNo;
    ...
    function JSvTBIDOCFilterPdtInTableTemp() {
        var value   = $("#oetTBIFrmFilterPdtHTML").val().toLowerCase();
        $("#otbTBIDocPdtAdvTableList tbody tr ").filter(function () {
    ...
    if (tUsrLevel != "HQ") {
        ...
        tWhere += " AND TCNMBranch.FTAgnCode IN(<?php echo $this->session->userdata('tSesUsrAgnCode'); ?>) ";
    } else {
        tWhere = "";
    }
    ...
    var oBrowse_BCH = {
        ...
        Join: {
            Table: ['TCNMBranch_L', 'TCNMWaHouse_L','TCNMAgency_L'],
        ...
        GrideView: {
            ...
            ColumnKeyLang: ['tBCHCode', 'tBCHName', 'tBCHBchType2'],
            ColumnsSize: ['15%', '30%', '35%'],
            WidthModal: 50,
            DataColumns: ['TCNMBranch.FTBchCode', 'TCNMBranch_L.FTBchName', 'TCNMAgency_L.FTAgnName', 'TCNMWaHouse_L.FTWahCode', 'TCNMWaHouse_L.FTWahName'],
            DataColumnsFormat: ['', '',''],
            DisabledColumns: [3, 4],
            Perpage: 10,
    ...
    $('#obtBrowseTWOBCHTo').click(function() {
    ...
    if (tUsrLevel != "HQ") {
        ...
        tWhereBchTo += " AND TCNMBranch.FTAgnCode IN(<?php echo $this->session->userdata('tSesUsrAgnCode'); ?>) ";
    } else {
        tWhereBchTo = "";
    }
