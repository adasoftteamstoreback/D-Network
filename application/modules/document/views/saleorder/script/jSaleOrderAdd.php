<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        if('<?=$tSORoute?>' == 'dcmSOEventEdit'){
            if('<?=$nAlwFindWahPCK?>' == 1 && nSOStaApv == 1){
    ...
    // Event Click Create Pick Document
        $('#obtSOCreatePCK').unbind().click(function(){
            var nStaSession = JCNxFuncChkSessionExpired();
            if(typeof(nStaSession) !== "undefined" && nStaSession == 1) {
                JSxSOCreatePCKDocument(false);
            }else{
    ...
    // Return Type : -
    function JSxSOCreatePCKDocument(pbIsConfirm){
        if(pbIsConfirm){
            ...
            if ($('#ocbPCKCheckWah').is(":checked")){
                var tChkWah             = 1;
            }
            
            if ($('#ocbPCKChkCat1').is(":checked")){
                var tChkCat1             = 1;
            }

            if ($('#ocbPCKChkCat2').is(":checked")){
                var tChkCat2             = 1;
            }

            if ($('#ocbPCKChkCat3').is(":checked")){
                var tChkCat3             = 1;
            }

            if ($('#ocbPCKChkCat4').is(":checked")){
                var tChkCat4             = 1;
            }

            if ($('#ocbPCKChkCat5').is(":checked")){
                var tChkCat5             = 1;
            }

            if('<?=$nStaPrcDoc?>' == null || '<?=$nStaPrcDoc?>' == 1 || '<?=$nStaPrcDoc?>' == '' || '<?=$nStaPrcDoc?>' == 3){
            JCNxOpenLoading();
                $.ajax({
                    type    : "POST",
                    ...
                    success: function(oResult){
                        ...
                        // console.log(aDataReturn['nStaItems'].length);

                        if(aDataReturn['nStaEvent'] == '1'){
                            JSxSODChkStaDocCallModalMQ();
                            // FSvCMNSetMsgErrorDialog('สร้างใบจัดเรียบร้อย');
                        }
                        else{
                            $("#odvTBodySOStockNotMatchList").empty();
                            for(var i = 0; i < aDataReturn['nStaItems'].length ; i++){          
                                var tHTML = '<tr>';
                                    tHTML += '<td>'+aDataReturn['nStaItems'][i]['FTPdtCode']+'</td>';
                                    tHTML += '<td>'+aDataReturn['nStaItems'][i]['FTPdtName']+'</td>';
                                    tHTML += '<td>'+aDataReturn['nStaItems'][i]['FCStkQty']+'</td>';
                                    tHTML += '<td>'+aDataReturn['nStaItems'][i]['FCDTQty']+'</td>';
                                    tHTML += '</tr>';
                                $('#odvTBodySOStockNotMatchList').append(tHTML);
                            }
                            $("#odvDBRModalStockNotMatch").modal('show');
                            JCNxCloseLoading();
                            // FSvCMNSetMsgErrorDialog('สต็อกสินค้าไม่เพียงพอ');
                            // JSvSOCallPageEditDoc($('#oetSODocNo').val());
                        }
                        JCNxCloseLoading();
                    },
            ...
            $('#odvPCKModalAppoveDoc').modal({backdrop:'static',keyboard:false});
    ...
    function JSxSODChkStaDocCallModalMQ(){

        // RESGEN+'_'+ DocNo +'_'+ User
    ...
    function JSxSOCallback(aitem){
        JSvSOCallPageEditDoc(aitem['rtDocNo']);
    }
