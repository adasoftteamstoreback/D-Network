<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#oetTXFrmFilterPdtHTML").attr("disabled", false);
	...
	var oTBBrowseWaHouseFrom      = function(poDataFnc){
		...
		// Check Branch
		if(typeof(tTBBchCode) != undefined && tTBBchCode != ""){
			tWhereModal += " "+tFindWahouse+" AND TCNMWaHouse.FTBchCode = '" + tTBBchCode + "' "; //TCNMWaHouse.FTWahRefCode = '" + tTBBchCode + "'
		}
	...
	var oTBBrowseWaHouseTo      = function(poDataFnc){
		...
		if(typeof(tTBBchCode) != undefined && tTBBchCode != ""){
			tWhereModal += " "+tFindWahouse+" AND TCNMWaHouse.FTBchCode = '" + tTBBchCode + "' ";
		}
	...
	var oTBBrowseBranchTo      = function(poDataFnc){
		...
		var oOptionReturn       = {
			...
			GrideView: {
				...
				ColumnsSize         : ['15%','35%','30%'],
				WidthModal          : 50,
				DataColumns         : ['TCNMBranch.FTBchCode','TCNMBranch_L.FTBchName', 'TCNMAgency_L.FTAgnName'],
				DataColumnsFormat   : ['','',''],
				Perpage             : 10,
	...
	$('#obtBrowseTWOBCH').click(function(){
		...
		if(tUsrLevel != "HQ"){
			tWhereModal 	+= " AND TCNMBranch.FTBchCode IN ("+tBchMulti+") ";
			tWhereModal 	+= " AND TCNMBranch.FTAgnCode IN(<?php echo $this->session->userdata('tSesUsrAgnCode'); ?>) ";
		}
