...
<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 p-t-10">
    <table class="table xWPdtTableFont" id="otbPAMDocRefTable">
        <thead>
            <tr class="xCNCenter">
        ...
        <tbody>
            ...
            if( $aDataDocHDRef['tCode'] == '1' ){
                ...
                foreach($aDataDocHDRef['aItems'] as $aValue){
                    ...
                    <tr class="xWDocRefItem" data-refdocno="<?=$aValue['FTXthRefDocNo']?>" data-alwdel="<?=$nStaAlwDelete?>" data-reftype="<?=$aValue['FTXthRefType']?>" data-refdocdate="<?=date_format(date_create($aValue['FDXthRefDocDate']),'Y-m-d')?>" data-refkey="<?=$aValue['FTXthRefKey']?>" >
                        ...
                        <td nowrap>
                            <?php
                                ...
                                if($aValue['FTXthRefType'] == 3){
                                    ...
                                    if ($tTitleDoc == 'TBO') {
                                        echo "ใบจ่ายโอน - สาขา";
                                    }else if ($aValue['FTXthRefKey'] == 'ABB') {
                                        echo "ใบขายสินค้า";
                                    }else {
                                        echo "ใบสั่งขาย";
                                    }
                        ...
                        <?php if($aValue['FTXthRefKey'] == 'ABB'){ ?>
                            <td nowrap>
                                <a onclick="JSvPAMJumptoABBPage('<?=$aValue['FTXthRefDocNo']?>')" style="cursor: pointer;color : #1866ae;text-decoration: underline;"><?=$aValue['FTXthRefDocNo']?></a>
                            </td>
                        <?php }else{ ?>
...
function JSvPAMJumptoABBPage(ptABBDocNo){
    
    JCNxOpenLoading();
    var nStaSession = JCNxFuncChkSessionExpired();
    if(typeof nStaSession !== "undefined" && nStaSession == 1) {
        JStCMMGetPanalLangSystemHTML("JSvABBCallPageEditDoc",ptABBDocNo);
        $.ajax({
            type    : "POST",
            url     : "docABB/0/0",
            data    : {'ptTypeJump' : '1',
