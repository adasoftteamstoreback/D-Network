<script type="text/javascript">
    ...
    $('#obtTWISubmitFromDoc').unbind().click(function() {
        ...
        if (typeof(nStaSession) !== "undefined" && nStaSession == 1) {
            ...
            var bCheckRmk = false;
            $("#otbTWIDocPdtAdvTableList tbody td.xCNTWIStaRmk").each(function( index ) {
                nSUMRmk = $(this).text();
                if(nSUMRmk != ''){
                    bCheckRmk = true;
                    return;
    ...
    function JSxTransferreceiptEventAddEdit(ptRoute) {
        $('#ofmTransferreceiptFormAdd').validate({
                ...
                submitHandler: function(form) {
                    ...
                    var nCountLot = 0;
                    var tLot      = $('.xWValuePdtBatchID').val()
                    // console.log( tLot );
                    
                    if( tLot !== undefined ){
                        $('.xWValuePdtBatchID').each(function(){
                            if( $(this).val() == '' ){
                                nCountLot += 1;
                            }
                        });
                    }
                    // console.log(nCountLot);
                    // return;

                    var tItem = $('#odvTWIDataPdtTableDTTemp #otbTWIDocPdtAdvTableList .xWPdtItem').length;
                    if ( tItem > 0 && (nCountLot > 0 && tLot !== undefined) ) {
                        FSvCMNSetMsgWarningDialog('กรุณากรอก LOT/BATCH No.');
                    }else if( tItem > 0 && nCountLot == 0 ){
                        $('#otaTWIFrmInfoOthRmk').removeAttr('disabled', true);
                        //วันที่ + เวลา
    ...
    // ptTable, ptField, pnSeq, pnQty
    // pnSeqNo, ptFieldName, ptValue
    function FSvTWIEditPdtIntoTableDT(ptTableName, ptFieldName, pnSeqNo, ptValue) {
        var nStaSession = JCNxFuncChkSessionExpired();
        if (typeof nStaSession !== "undefined" && nStaSession == 1) {
            var tTWIDocNo = $("#oetTWIDocNo").val();
            var tTWIBchCode = $("#oetSOFrmBchCode").val();
