<div class="xCNtableFixHead">
    <table class="table xRefIntTable" id="otbDBRRefPoDocDetail">
        <thead >
            <tr>
            ...
            <?php if($aDataList['rtCode'] == 1 ):?>
                <?php foreach($aDataList['raItems'] AS $nKey => $aValue): ?>
                    <?php 
                        ...
                        $tTextClassDisable  = "";
                        if($aValue['FCXsdQty'] == '0'){
                            $tTextDisable       = "disabled";
                            $tTextClassDisable  = "xCNDocDisabled";
                        }  
                    ?>
                    <tr class="xCNTextDetail2 xCNCheckQTY" 
                        data-qtyleft="<?=number_format(@$aValue['FCXsdQty'],0);?>"
                        data-qtyall="<?=number_format(@$aValue['FCXsdQtyAll'],0);?>"
                    >
                        <td class="xRefIntTd text-center">
                            <label class="fancy-checkbox ">
                                <input type="checkbox" class="ocbRefIntDocDT" name="ocbRefIntDocDT[]" value="<?=$aValue['FNXsdSeqNo']?>" <?=@$tTextDisable;?>>
                        ...
                        <td class="xRefIntTd text-right"><?=number_format($aValue['FCXsdQtyAll'],$nOptDecimalShow)?></td>
...
$('#ocbRefIntDocDTAll').click(function(){
    if($(this).is(':checked')==true){
        $('#otbDBRRefPoDocDetail > tbody  > tr').each(function() {
