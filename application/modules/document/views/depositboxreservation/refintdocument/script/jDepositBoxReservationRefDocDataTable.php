...
$('.xPurchaseInvoiceRefInt').click(function(){
    ...
    JSxDBRCallRefIntDocDetailDataTable(tBchCode,tDocNo,tdoctype);
...
function JSxDBRCallRefIntDocDetailDataTable(ptBchCode,ptDocNo,ptdoctype){
    JCNxOpenLoading();
    $.ajax({
        type: "POST",
        url: "docDBRCallRefIntDocDetailDataTable",
        data: {
            'ptBchCode'     : ptBchCode,
            'ptDocNo'       : ptDocNo,
            'ptdoctype'     : ptdoctype
        ...
        success: function (oResult){
            $('#odvDBRRefIntDocDetail').html(oResult);
            JCNxCloseLoading();
        },
