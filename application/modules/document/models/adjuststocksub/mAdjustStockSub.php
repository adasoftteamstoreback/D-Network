...
class mAdjustStockSub extends CI_Model
{
    ...
    public function FSnMAdjStkSubDelDTTmp($paData)
    {
        try {
            ...
            $this->db->where('FNXtdSeqNo>', $paData['FNXtdSeqNo']);
            $this->db->where('FTXthDocKey', $paData['FTXthDocKey']);
            $this->db->set('FNXtdSeqNo', "FNXtdSeqNo -1", FALSE);
            $this->db->update('TCNTDocDTTmp'); 


            $this->db->where('FTXshDocNo', $paData['FTAjhDocNo']);
            ...
            $this->db->where('FNXsdSeqNo>', $paData['FNXtdSeqNo']);
            $this->db->where('FTXthDocKey', $paData['FTXthDocKey']);
            $this->db->set('FNXsdSeqNo', "FNXsdSeqNo -1", FALSE);
            $this->db->update('TCNTDocDTFhnTmp'); 
    ...
    public function FSaMASTEventAddProducts($paDataCondition, $paDataInsert)
    {
        ...
        // $this->db->query($tSQL);
        $oQuery = $this->db->query($tSQLCallPDt);
        // if ($this->db->trans_status() === FALSE) {
        //     $aReturn = array(
        //         'tSQL'      => $tSQL,
        //         'aPDtList' => array(),
    ...
    public function FSaMASTEventAddProductsFashion($paInsertData,$pnOptionAddPdt){
        try {
        ...
        // Create By Nattakit(Nale) 2021/05/13
    // Insert PdtFhn To DTfhnTemp
    public function FSaMASTEventUpdateProductsFashion($paInsertData,$pnOptionAddPdt){
        try {
    ...
    public function FSaMAdustStockGetLastSeqDT($aDataPdtWhere){

        $tBchCode = $aDataPdtWhere['FTBchCode'];
        $tAjhDocNo = $aDataPdtWhere['FTAjhDocNo'];
        $tSessionID = $aDataPdtWhere['FTSessionID'];

        $tSQL = "SELECT TOP 1
                        ISNULL(FNXtdSeqNo,0) AS FNXtdSeqNo
                    FROM
                        TCNTDocDTTmp
                    WHERE
                        FTSessionID = '$tSessionID'
                    AND FTXthDocKey = 'TCNTPdtAdjStkHD'
                    AND FTXthDocNo = '$tAjhDocNo'
    ...
    public function FSaMAdustStockGetLastSeqDTFhn($aDataPdtWhere){
        ...
        $tSQL = "SELECT TOP 1
                        ISNULL(FNXsdSeqNo,0) AS FNXsdSeqNo
                    FROM
                        TCNTDocDTFhnTmp
                    WHERE
                        FTSessionID = '$tSessionID'
                    AND FTXthDocKey = 'TCNTPdtAdjStkHD'
                    AND FTXshDocNo = '$tAjhDocNo'
                    AND FTBchCode = '$tBchCode'
                    ORDER BY FNXsdSeqNo DESC
                    ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            $aReturn = array(
        ...
        if ($paDataWhere['nAdjStkSubOptionAddPdt'] == 1) {
            ...
            if ($oQuery->num_rows() > 0) {
                ...
                $tSQL = "UPDATE TCNTDocDTTmp SET
                        FCAjdUnitQtyC1 = '" . ($aResult["FCAjdUnitQtyC1"] + 0) . "',
                        FCAjdQtyAllC1 = '" . (($aResult["FCAjdUnitQtyC1"] + 0) * $aResult["FCPdtUnitFact"]) . "'
                        WHERE 
                        FTBchCode    = '" . $paDataWhere['FTBchCode'] . "' AND
                        FTXthDocNo   = '" . $paDataWhere['FTAjhDocNo'] . "' AND
                        ISNULL(FTAjdPlcCode,'')   = '" . $paData['FTAjdPlcCode'] . "' AND
                ...
                $this->db->insert('TCNTDocDTTmp', array(
                    ...
                    'FTAjdPlcCode'      => $paData['FTAjdPlcCode'],
                    'FCPdtUnitFact'     => $paData['FCPdtUnitFact'],
