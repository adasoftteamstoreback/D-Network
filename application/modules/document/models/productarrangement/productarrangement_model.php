...
class productarrangement_model extends CI_Model {
    ...
    public function FSaMPAMGetDataTableList($paDataCondition){
        ...
        $tSQL2  = "  SELECT
                    ...
                    LEFT JOIN TCNTPdtPickHDDocRef   HDREF   WITH (NOLOCK) ON HD.FTXthDocNo    = HDREF.FTXthDocNo  AND HDREF.FTXthRefType    = '11'
    ...
    public function FSaMCENDeleteDocRefInTmp(){
        $tPAMDocKey ='TCNTPdtPickHD';
        $tSessionID = $this->session->userdata('tSesSessionID');
        $this->db->where_in('FTSessionID', $tSessionID);
    ...
    public function FSoMPAMCallRefIntDocDataTable($paDataCondition){
        ...
        if(isset($tPAMRefIntBchCode) && !empty($tPAMRefIntBchCode)){
          ...
          if ($this->session->userdata("tSesUsrLevel") != 'HQ') {
            ...
            $tSQLMain .= " AND POHD.FTBchCode IN ($tSesUsrBchCodeMulti) ";
          }
        }
    ...
    // Function: Get Data PAM HD List
    public function FSoMPAMCallRefIntDocSaDataTable($paDataCondition){
        $aRowLen                = FCNaHCallLenData($paDataCondition['nRow'],$paDataCondition['nPage']);
        $nLngID                 = $paDataCondition['FNLngID'];
        $aAdvanceSearch         = $paDataCondition['aAdvanceSearch'];
        // Advance Search
        ...
        $tSQLMain = "   SELECT
                                ...
                                BCHLTO.FTBchName AS BCHNameTo ,
                                A.SumA
                            FROM TARTSoHD           SAHD    WITH (NOLOCK)
                            LEFT JOIN   (   select
                                                FTXshDocNo,
                                                SUM(FCXsdQtyLef) AS SumA
                                                from TARTSoDT
                                                group by FTXshDocNo
                                        ) A ON A.FTXshDocNo = SAHD.FTXshDocNo
                            LEFT JOIN TCNMBranch_L  BCHL    WITH (NOLOCK) ON SAHD.FTBchCode     = BCHL.FTBchCode    AND BCHL.FNLngID    = $nLngID
                            LEFT JOIN TCNMBranch_L  BCHLTO  WITH (NOLOCK) ON SAHD.FTBchCode    = BCHLTO.FTBchCode  AND BCHLTO.FNLngID    = $nLngID
        ...
        if(isset($tPAMRefIntBchCode) && !empty($tPAMRefIntBchCode)){
            $tSQLMain .= " AND (SAHD.FTBchCode = '$tPAMRefIntBchCode' OR SAHD.FTBchCode = '$tPAMRefIntBchCode')";
        }else {
          if ($this->session->userdata("tSesUsrLevel") != 'HQ') {
            $tSesUsrBchCodeMulti = $this->session->userdata("tSesUsrBchCodeMulti");
        ...
        if(isset($tPAMRefIntDocNo) && !empty($tPAMRefIntDocNo)){
            $tSQLMain .= " AND (SAHD.FTXshDocNo LIKE '%$tPAMRefIntDocNo%')";
        }

        // ค้นหาจากวันที่ - ถึงวันที่
        if(!empty($tPAMRefIntDocDateFrm) && !empty($tPAMRefIntDocDateTo)){
            $tSQLMain .= " AND ((SAHD.FDXshDocDate BETWEEN CONVERT(datetime,'$tPAMRefIntDocDateFrm 00:00:00') AND CONVERT(datetime,'$tPAMRefIntDocDateTo 23:59:59')) OR (SAHD.FDXphDocDate BETWEEN CONVERT(datetime,'$tPAMRefIntDocDateTo 23:00:00') AND CONVERT(datetime,'$tPAMRefIntDocDateFrm 00:00:00')))";
        }

        // ค้นหาสถานะเอกสาร
        if(isset($tPAMRefIntStaDoc) && !empty($tPAMRefIntStaDoc)){
            if ($tPAMRefIntStaDoc == 3) {
                $tSQLMain .= " AND SAHD.FTXshStaDoc = '$tPAMRefIntStaDoc'";
            } elseif ($tPAMRefIntStaDoc == 2) {
                $tSQLMain .= " AND ISNULL(SAHD.FTXshStaApv,'') = '' AND SAHD.FTXshStaDoc != '3'";
            } elseif ($tPAMRefIntStaDoc == 1) {
                $tSQLMain .= " AND SAHD.FTXshStaApv = '$tPAMRefIntStaDoc'";
            }
        }
    ...
    // Functionality: Get Data Purchase Order HD List
    public function FSoMPAMCallRefIntDocDT2DataTable($paData){
    ...
    public function FSoMPAMCallRefInt2DocInsertDTToTemp($paData){
    ...
    public function FSxMPAMCheckDocRef($paData){
        ...
        if( $tDocType == '1' ){ // กรณีใบจัดสินค้าที่เกิดจากใบจ่ายโอน-สาขา
            ...
            $tSQL = "   SELECT PCKAPV.FTXthDocNo,PCKAPV.FTXthStaApv,PCK.FTXthRefDocNo
                        ...
                        AND (PCK.FTXthRefKey = 'PdtTbo' OR PCK.FTXthRefKey = 'TDO')
    ...
    public function FSxMPAMUpdTboOnCancelOrDelete($paData){
        ...
        if ( $oQuery->num_rows() == 0 ){
            $tSQL1 = "  SELECT PICKREF.FTXthRefDocNo
                        ...
                        AND PICKREF.FTXthRefKey IN ('PdtTbo','TDO') ";
            $oQuery1 = $this->db->query($tSQL1);
            if ( $oQuery1->num_rows() > 0 ){
                $tDocNoTBO  = $oQuery1->row_array()['FTXthRefDocNo'];
                $this->db->set('FDLastUpdOn',$dDate);
    ...
    public function FSbMPAMChkDocRefAlwCancel($paData){
        ...
        if( $tDocType == '1' ){
            ...
            $tSQL = "   SELECT SALE.FTXshDocNo, SALE.FTChnCode
                        FROM TCNTPdtPickHDDocRef	PICK WITH(NOLOCK)
            ...
            $oQuery = $this->db->query($tSQL);
            if ( $oQuery->num_rows() > 0 ){
                $tSaleChnCode   = $oQuery->row_array()['FTChnCode'];
