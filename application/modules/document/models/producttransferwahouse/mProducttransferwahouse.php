...
class mProducttransferwahouse extends CI_Model
{
        ...
        $tSQL   =   "
            SELECT TOP ". get_cookie('nShowRecordInPageList')." c.* FROM(
                ...
                * FROM( 
                    ...
                    WHERE TFW.FDCreateOn <> '' AND ISNULL(TFW.FTXthDocType,'') = ''
        ";
    ...
    function FSnMTFWUpdateInlineDTTemp($aDataUpd, $aDataWhere)
    {
        try {
            ...
            $this->db->set('FCXtdQty', $aDataUpd['FCXtdQty']);
    ...
    */
    ...
    public function FSaMTFWGetHD($paData){
        ...
        $tSQL       = "
            SELECT
                CASE WHEN TFW.FTXthRefInt <> '' AND TFW.FTXthRefInt IS NOT NULL THEN 2 ELSE 1 END AS FTXthRefInType,
                TFW.FTBchCode,
                TFW.FTXthMerCode,
                MCHL.FTMerName,
                TFW.FTXthDocNo,
            ...
            FROM [TCNTPdtTwxHD] TFW WITH (NOLOCK)
            LEFT JOIN TCNMMerchant_L MCHL WITH (NOLOCK)   ON TFW.FTXthMerCode = MCHL.FTMerCode AND MCHL.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMBranch_L BCHL WITH (NOLOCK)   ON TFW.FTBchCode   = BCHL.FTBchCode AND BCHL.FNLngID    = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMUser_L USRL WITH (NOLOCK)   ON TFW.FTCreateBy   = USRL.FTUsrCode AND USRL.FNLngID     = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMUser_L USRAPV WITH (NOLOCK) ON TFW.FTXthApvCode = USRAPV.FTUsrCode AND USRAPV.FNLngID = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMUsrDepart_L DPTL WITH (NOLOCK)   ON TFW.FTDptCode = DPTL.FTDptCode AND DPTL.FNLngID   = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMShop FSHP WITH (NOLOCK)   ON TFW.FTXthShopFrm = FSHP.FTShpCode AND TFW.FTBchCode      = FSHP.FTBchCode
            LEFT JOIN TCNMShop_L FSHPL WITH (NOLOCK)  ON TFW.FTXthShopFrm = FSHPL.FTShpCode AND TFW.FTBchCode   = FSHPL.FTBchCode AND FSHPL.FNLngID     = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMShop TSHP WITH (NOLOCK)   ON TFW.FTXthShopTo = TSHP.FTShpCode AND TFW.FTBchCode       = TSHP.FTBchCode
            LEFT JOIN TCNMShop_L TSHPL WITH (NOLOCK)  ON TFW.FTXthShopTo = TSHPL.FTShpCode AND TFW.FTBchCode    = TSHPL.FTBchCode AND TSHPL.FNLngID     = ".$this->db->escape($nLngID)."
            ...
            LEFT JOIN TCNMWaHouse_L WAHLF WITH (NOLOCK)  ON TFW.FTXthWhFrm = WAHLF.FTWahCode AND TFW.FTBchCode  = WAHLF.FTBchCode  AND WAHLF.FNLngID    = ".$this->db->escape($nLngID)."
            ...
            LEFT JOIN TCNMWaHouse_L WAHLT WITH (NOLOCK)  ON TFW.FTXthWhTo = WAHLT.FTWahCode AND TFW.FTBchCode   = WAHLT.FTBchCode AND WAHLT.FNLngID     = ".$this->db->escape($nLngID)."
