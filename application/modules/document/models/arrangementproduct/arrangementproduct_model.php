...
class arrangementproduct_model extends CI_Model {
    ...
    // เปลี่ยนสถานะ PrcDoc
    public function FSaMPCKStaprcDocument($paDataUpdate){
        $tDocno = $paDataUpdate['FTXthDocNo'];

        //อัพเดท Staprc
        $tSQL   = " UPDATE TARTSoHD WITH(ROWLOCK)
                SET FTXshStaPrcDoc = '3' 
                FROM TARTSoHD HD 
                INNER JOIN (
                    SELECT DISTINCT FTXthRefDocNo FROM TCNTPdtPickHDDocRef WHERE FTXthDocNo = '".$tDocno."'
    ...
    public function FSxMPAMCancelDocument($paData){
        ...
        $tSQLGetSO = " SELECT TOP 1 FTXshDocno from TARTSoHDDocRef  where FTXshRefDocNo = '$docno'";
            $oQueryGetSO = $this->db->query($tSQLGetSO);
            $aItemsSO = $oQueryGetSO->result_array();
            $tSODocNo = $aItemsSO[0]['FTXshDocno'];

        $tSQLChkDocref = " SELECT * from TARTSoHDDocRef where FTXshDocNo = '$tSODocNo' AND FTXshRefKey = 'CSTPICK'";
            $oQueryChk = $this->db->query($tSQLChkDocref);
            $aItemsChk = $oQueryChk->result_array();
        ...
        if($paData['tDocType'] == '11' ){ //ใบจัดสินค้าให้ลูกค้า - อ้างใบจ่ายโอนสาขา
            ...
            $oQueryChk = $this->db->query($tSQLChkDocref);
            $aItemsChk = $oQueryChk->result_array();
            if(count($aItemsChk) == 0){
                $this->db->set('FTXshStaPrcDoc',NULL); //ยกเลิก
                $this->db->where('FTXshDocNo',$tSODocNo);
    ...
    public function FSxMPAMRelationSO($ptPICKDocNo){
        ...
        if($aItems[0]['FNCountWDocApv'] == 0){
            ...
            //หากจัดครบแล้วจะ Gen
            $tSQLGetSO = "   SELECT HD.FTBchCode,HD.FTXshDocNo
                        FROM TARTSoHD HD
                        INNER JOIN (
                        SELECT DISTINCT FTXthRefDocNo FROM TCNTPdtPickHDDocRef WHERE FTXthDocNo = '".$ptPICKDocNo."'
                        ) REF ON REF.FTXthRefDocNo = HD.FTXshDocNo ";
            $oQuery = $this->db->query($tSQLGetSO);
            $aItemsSO = $oQuery->result_array();

            $aMQParams = [
                "tVhostType" => "I",
                "queueName" => "GEN_ABB",
                "params"    => [
                    'ptFunction'    => 'TPSTSalHD',
                    'ptSource'      => 'AdaStoreBack',
                    'ptDest'        => 'MQAdaLink',
                    'ptData'        => json_encode([
                        "ptBchCode"     => $aItemsSO['0']['FTBchCode'],
                        "ptDocNo"       => $aItemsSO['0']['FTXshDocNo'],
                        "ptDocType"     => '1',
    ...
    // เช็คสต็อกก่อนส่ง MQ
    public function FSaMPCKCheckStk($ptPCKDocNo){
    ...
    FROM
        ...
        SELECT
            ...
            HD.FTWahCode,
            DT.FTPdtCode,
            SUM ( DT.FCXtdQtyAll ) AS FCDTQty 
        FROM
            TCNTPdtPickDT DT WITH ( NOLOCK ) INNER JOIN TCNTPdtPickHD HD WITH ( NOLOCK ) ON HD.FTXthDocNo = DT.FTXthDocNo 
        WHERE
            DT.FTXthDocNo = '$ptPCKDocNo' 
        GROUP BY
            DT.FTPdtCode,HD.FTBchCode,HD.FTWahCode
        ) DT
        LEFT JOIN TCNTPdtStkBal STK WITH ( NOLOCK ) ON STK.FTPdtCode = DT.FTPdtCode 
        AND STK.FTBchCode = DT.FTBchCode 
        AND STK.FTWahCode = DT.FTWahCode 
    WHERE
        ( ISNULL( STK.FCStkQty, 0 ) - ISNULL( DT.FCDTQty, 0 ) ) < 0 
    ORDER BY
        DT.FTPdtCode ASC";
    ...
    public function FSaMPAMGetDataStaBooking($ptDocNo, $ptRefDocNo)
    {
        $tSQLPL = "SELECT * 
                    FROM TCNTPdtPickHDDocRef 
                   WHERE FTXthDocNo = '$ptDocNo' 
                   AND FTXthRefKey = 'ABB'";

        $oQueryPL = $this->db->query($tSQLPL);
        if ( $oQueryPL->num_rows() > 0 ){
            $tSQLBooking = "SELECT * 
                                FROM TARTSoHDDocRef 
                            WHERE FTXshDocNo = '$ptRefDocNo' 
                            AND FTXshRefKey = 'RTBook'";
            $oQueryBooking = $this->db->query($tSQLBooking);
            if ( $oQueryBooking->num_rows() > 0 ){
                $aResult = array(
                    'nStaEvent' => '900',
                    'tStaMessg' => 'มีใบจองแล้ว'
                );
            $oQueryPL = $this->db->query($tSQLPL);
            }else{
                $aResult = array(
                    'nStaEvent' => '1',
                    'tStaMessg' => 'ยังไม่มีใบจอง'
                );
            }
    ...
    public function FSaMPAMGetDataHDSORefTmp($paData){
        $tTableTmpHDRef = $paData['tTableTmpHDRef'];
        $FTXthDocNo     = $paData['FTXthDocNo'];
        $FTXthDocKey    = $paData['FTXthDocKey'];
        $FTSessionID    = $paData['FTSessionID'];

        $tSQL = "   SELECT TMP.FTXthRefDocNo 
                    FROM $tTableTmpHDRef TMP WITH(NOLOCK)
                    WHERE TMP.FTXthDocNo  = '$FTXthDocNo'
                      AND TMP.FTXthDocKey = '$FTXthDocKey'
                      AND TMP.FTSessionID = '$FTSessionID'
                      AND TMP.FTXthRefKey = 'SO'
                    ORDER BY TMP.FDCreateOn DESC ";
        $oQuery = $this->db->query($tSQL);
        if ( $oQuery->num_rows() > 0 ){
            $aResult    = array(
