    ...
    public function FSaMPIGetDataPdtByBarCode($paDataPdtParams){
        ...
        $nLngID     = $paDataPdtParams['nLngID'];
                $tSQL ="SELECT
                    ...
                    LEFT JOIN TCNMPdtSpcBch SpcBch WITH (NOLOCK) ON PDTBAR.FTPdtCode = SpcBch.FTPdtCode
                    LEFT JOIN TCNMBranch BCH WITH (NOLOCK) ON SpcBch.FTAgnCode = BCH.FTAgnCode
                    LEFT JOIN (
                        SELECT DISTINCT
                            FTVatCode,
                            FCVatRate,
            ...
            if($this->session->userdata('tSesUsrAgnCode')!=''){
                ...
                $tSQL   .= " AND SpcBch.FTAgnCode = '$tSesUsrAgnCode' ";
            }
            
            if(!empty($paDataPdtParams['tBchCode'])){
                $tBchCode = $paDataPdtParams['tBchCode'];
                $tSQL   .= " AND BCH.FTAgnCode = '$tBchCode' ";
            }
