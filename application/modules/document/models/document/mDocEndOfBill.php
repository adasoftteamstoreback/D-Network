...
class mDocEndOfBill extends CI_Model {
    /**
     * 
     * @param type $paParams
     * @return string
    ...
    public function FSaMDOCEndOfBillGetDTSumVat($paParams){
        ...
        $tSQL = "   SELECT
                    ...
                    FROM $tTableDTTmp DOCTMP WITH (NOLOCK)
                    WHERE DOCTMP.FTSessionID ='$tSessionID' AND DOCTMP.FTXthDocKey = '$tDocKey' AND DOCTMP.FTXthDocNo = '$tDocNo' 
    ...
    public function FSaMDOCEndOfBillGetHDDisTmp($paParams){
        ...
        $tSQL = "   SELECT 
                        ...
                        STUFF((
                            ...
                            FROM $tTableHDDisTmp DOCCONCAT WITH (NOLOCK)
                            WHERE  1=1 
                        ...
                        STUFF((
                            SELECT  ',' + DOCCONCATType.FTXtdDisChgType
                            FROM $tTableHDDisTmp DOCCONCATType WITH (NOLOCK)
                            WHERE  1=1 
                            AND DOCCONCATType.FTBchCode 		= '".$tBchCode."'
                            AND DOCCONCATType.FTXthDocNo		= '".$tDocNo."'
                            AND DOCCONCATType.FTSessionID		= '".$tSessionID."'
                        FOR XML PATH('')), 1, 1, '') AS FTXtdDisChgType
                    FROM $tTableHDDisTmp DOCHDDISTMP WITH (NOLOCK)
