...
class mTransferwarehouseout extends CI_Model {
    ...
    public function FSaMTWOList($paData){
        ...
        $tSQL       = "
                SELECT TOP ".get_cookie('nShowRecordInPageList')." 
                    ...
                    SELECT * FROM
                        ...
                        LEFT JOIN TCNMBranch_L  BCHL WITH (NOLOCK) ON TWO.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID   = ".$this->db->escape($nLngID)." 
                        LEFT JOIN TCNMUser_L    USRL WITH (NOLOCK) ON TWO.FTUsrCode = USRL.FTUsrCode AND USRL.FNLngID   = ".$this->db->escape($nLngID)."
                        LEFT JOIN TCNMUser_L    USRLAPV WITH (NOLOCK) ON TWO.FTXthApvCode = USRLAPV.FTUsrCode AND USRLAPV.FNLngID = ".$this->db->escape($nLngID)."
                        WHERE TWO.FDCreateOn <> '' AND TWO.FNXthDocType = ".$this->db->escape($nDocType)."
        ";
        ...
        if (@$tSearchList != '') {
            $tSQL   .= " 
                AND (
                    (TWO.FTXthDocNo LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                    OR (CONVERT(CHAR(10),TWO.FDXthDocDate,103) LIKE '%".$this->db->escape_like_str($tSearchList)."%')
                )
            ";
        }
        ...
        if (!empty($tSearchBchCodeFrom) && !empty($tSearchBchCodeFrom)) {
            $tSQL   .= "
                AND (
                    (TWO.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeFrom)."   AND ".$this->db->escape($tSearchBchCodeTo).")
                    OR (TWO.FTBchCode BETWEEN ".$this->db->escape($tSearchBchCodeTo)."  AND ".$this->db->escape($tSearchBchCodeFrom).")
                )
            ";
        }
        ...
        if (!empty($tSearchDocDateFrom) && !empty($tSearchDocDateTo)) {
            $tSQL   .= "
                AND (
                    (TWO.FDXthDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 00:00:00").") AND CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 23:59:59")."))
                    OR (TWO.FDXthDocDate BETWEEN CONVERT(datetime,".$this->db->escape($tSearchDocDateTo." 23:00:00").") AND CONVERT(datetime,".$this->db->escape($tSearchDocDateFrom." 00:00:00")."))
                )
            ";
        }
        ...
        if (!empty($tSearchStaDoc) && ($tSearchStaDoc != "0")) {
            if ($tSearchStaDoc == 3) {
                $tSQL   .= " AND TWO.FTXthStaDoc = ".$this->db->escape($tSearchStaDoc)." ";
            } else if ($tSearchStaDoc == 2) {
                $tSQL   .= " AND ISNULL(TWO.FTXthStaApv,'') = '' AND TWO.FTXthStaDoc != '3'";
            } else if ($tSearchStaDoc == 1) {
                $tSQL   .= " AND TWO.FTXthStaApv = ".$this->db->escape($tSearchStaDoc)." ";
            }
        }
        ...
        if (!empty($tSearchStaPrcStk) && ($tSearchStaPrcStk != "0")) {
            if ($tSearchStaPrcStk == 3) {
                $tSQL   .= " AND (TWO.FTXthStaPrcStk = ".$this->db->escape($tSearchStaPrcStk)." OR ISNULL(TWO.FTXthStaPrcStk,'') = '') ";
            } else {
                $tSQL   .= " AND TWO.FTXthStaPrcStk  = ".$this->db->escape($tSearchStaPrcStk)." ";
            }
        }
        ...
        if (!empty($tSearchStaDocAct) && ($tSearchStaDocAct != "0")) {
            if ($tSearchStaDocAct == 2) {
                ...
                $tSQL   .= " AND TWO.FNXthStaDocAct = ".$this->db->escape($tSearchStaDocAct)." ";
            }
        }
    ...
    public function FSaMTwoUpdateRmk($paDataUpdate){
    ...
    //เชคเอกสารว่ามีอ้างอิงไป ขาเข้าไหม
    public function FSaMTwoCheckIn($paDataWhere){
        $tCheckIn = $paDataWhere['FTXthDocNo'];

        $tsql  =  "SELECT Twi.FTXthRefInt from TCNTPdtTwiHD Twi  
                    where Twi.FTXthRefInt = '$tCheckIn'";
    ...
    public function FSaMTWOGetDataDocHD($paDataWhere){
        ...
        $tSQL       = " 
            ...
            LEFT JOIN TCNMShop_L        SHPL    WITH (NOLOCK)   ON DOCHD.FTXthShopFrm   = SHPL.FTShpCode	AND SHPL.FNLngID    = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMShop_L        SHPL_T  WITH (NOLOCK)   ON DOCHD.FTXthShopTo    = SHPL_T.FTShpCode	AND SHPL_T.FNLngID  = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMMerchant_L    MERL    WITH (NOLOCK)   ON SHP.FTMerCode        = MERL.FTMerCode	AND MERL.FNLngID    = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMWaHouse_L     WAHL    WITH (NOLOCK)   ON DOCHD.FTXthWhFrm     = WAHL.FTWahCode    AND BCHL.FTBchCode  = WAHL.FTBchCode   AND WAHL.FNLngID     = ".$this->db->escape($nLngID)."
            LEFT JOIN TCNMWaHouse_L     WAHL_T  WITH (NOLOCK)   ON DOCHD.FTXthWhTo      = WAHL_T.FTWahCode	AND BCHL.FTBchCode  = WAHL_T.FTBchCode AND WAHL_T.FNLngID   = ".$this->db->escape($nLngID)."
            ...
            LEFT JOIN TCNMCst_L         CSTL    WITH (NOLOCK)   ON DOCHD.FTCstCode		= CSTL.FTCstCode	AND CSTL.FNLngID	= ".$this->db->escape($nLngID)."
            WHERE 1=1 AND DOCHD.FTXthDocNo  = ".$this->db->escape($tTWODocNo)." 
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
            $aDetail = $oQuery->row_array();
    ...
    public function FSaMTWOGetPdtInTmpForSendToAPI($paData){
        ...
        $tSQL       = " SELECT
                            ...
                            '$tDocCode'                 AS ptDocNo,
                            ''                          AS ptRefDocNo
                        FROM TCNTPdtTwoDT               TMP WITH(NOLOCK)
    ...
    public function FSxMTWOFindTextNamePDTNoStock($ptTextCodePDT){
    ...
    public function FSxMTWOChkConfig(){
    ...
    //Get ข้อมูล API
    public function FSaMTWOGetConfigAPI(){
    ...
    public function FSxMTWOUpdatePdtStkPrc($paDataWhere,$paHavePdtInWah){
        $tBchCode   = $paDataWhere['FTBchCode'];
        ...
        $this->db->update('TCNTPdtTwoDT');


        return $this->db->last_query();
    }

    public function FSxMTWOUpdatePdtStkPrcAll($paDataWhere,$ptStatus){
        $tBchCode   = $paDataWhere['FTBchCode'];
        $tDocCode   = $paDataWhere['FTXshDocNo'];
        $tDocKey    = $paDataWhere['FTXthDocKey'];
        $tSessionID = $paDataWhere['FTSessionID'];
