...
<div id="odvPshRowNavMenu" class="row">
		<div class="custom-tabs-line tabs-line-bottom left-aligned">
			<ul class="nav" role="tablist">
				<!--ข้อมูลหลัก-->
				<li id="oliPSHDetail" class="xWMenu active" data-menutype="PS" onclick="" style="cursor: pointer">
				...
				<!--ตั้งค่าเอเอาท์-->
				<li id="oliPSHDetail" class="xWMenu Virtual" data-menutype="ST" onclick="JSxGetPSHContentLayoutSetting()" style="cursor: pointer">
					<a role="tab" data-toggle="tab" data-target="#odvPSHContentInfoST" aria-expanded="true"><?php echo language('pos/posshop/posshop','ตั้งค่าช่องฝาก')?></a>
				</li>
				<!--สั่ง/จ่าย-->
				...
				<!--ตรวจสอบสถานะ-->
				<li id="oliPSHDetail" class="xWMenu Virtual" data-menutype="CS" onclick="JSxGetPSHContentCheckStatus()" style="cursor: pointer">
				...
				<!--ใบปรับสถานะ-->
				<li id="oliPSHDetail" class="xWMenu Virtual" data-menutype="CG" onclick="JSxGetPSHContentAdjustStatus()" style="cursor: pointer">
            ...
            <form class="validate-form" action="javascript:void(0)" method="post" enctype="multipart/form-data" id="ofmAddPosShop">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12  text-right">
                        <?php if($aAlwEventPosShop['tAutStaFull'] == 1 || ($aAlwEventPosShop['tAutStaAdd'] == 1 || $aAlwEventPosShop['tAutStaEdit'] == 1)) : ?>
                                <button type="submit" class="btn xCNBTNSubSave" id="obtAddPosShop"  onclick="JSoAddPosShop('<?= $tRoute?>')"> <?php echo  language('common/main/main', 'tSave')?></button>
                        <?php endif; ?>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-xs-5 col-sm-5 col-md-5 col-lg-5">
                        ...
                        <label class="xCNLabelFrm">
                            <span style="color:red">*</span>
                            <?= language('pos/posshop/posshop','tPshType')?>
                        </label>
                        <select class="selectpicker form-control" id="ocmPshStaType" name="ocmPshStaType" maxlength="1" data-validate="<?= language('pos/posshop/posshop','tPSHValidocmPshStaUse')?>" onchange="JSxChangeStaType()">
                            <!-- <option value="" checked><?=language('pos/posshop/posshop','tPshSelectSta')?></option> -->
                            <option value="1"><?=language('pos/posshop/posshop','tPshTypePhysical')?></option>
                            <option value="2"><?=language('pos/posshop/posshop','tPshTypeVirtual')?></option>
                        </select>
                        </div>
                    </div>
                    </div>
                    ...
                    <div class="col-xs-5 col-sm-5 col-md-5 col-lg-5">
                        ...
                        <select class="selectpicker form-control" id="ocmPshStaUse" name="ocmPshStaUse" maxlength="1" data-validate="<?= language('pos/posshop/posshop','tPSHValidocmPshStaUse')?>">
                            ...
                            <!-- ซ่อนส่วน SetLayout แต่ให้ทำการเก็บค่า default ตามปกติ -->
    ...
    <div class="modal fade" id="odvModalStaType">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>มีการเปลี่ยนชนิดตู้ฝากเป็นจุดบริการฝาก ข้อมูลสถานะจะถูกล้าง ยืนยันที่จะเปลี่ยน</p>
                </div>
                <div class="modal-footer">
                    <button id="osmConfirmStaType" type="button" class="btn xCNBTNPrimery" onClick="JSxSMSDeleteMutirecord()"><?=language('common/main/main', 'tModalConfirm')?></button>
                    <button id="osmCancelStaType" type="button" class="btn xCNBTNDefult" data-dismiss="modal"><?=language('common/main/main', 'tModalCancel')?></button>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function JSxChangeStaType() {
        var nStaType = $('#ocmPshStaType').val();
        if (nStaType == 2) {
            $('#odvModalStaType').modal('show')
            //ยืนยัน
            $( "#osmConfirmStaType" ).click(function() {
                $('.Virtual').hide();
                $("#ocmPshStaType").val("2").selectpicker('refresh');
                $('#odvModalStaType').modal('hide')
            }); 
            //ยกเลิก
            $( "#osmCancelStaType" ).click(function() {
                $("#ocmPshStaType").val("1").selectpicker('refresh');
                $('#odvModalStaType').modal('hide')
            }); 
