...
<script type="text/javascript">
    ...
    function JSvCallPagePosShopEventAdd(){
        ...
        var tChkShopLayout  = $('#oetPshChkShopLayout').val();
        if (tChkShopLayout == 1) {
            $.ajax({
            type: "POST",
            url: "posshoppageadd",
            ...
            FSvCMNSetMsgErrorDialog('ไม่สามารถเพิ่มเครื่องจุดขาย เนื่องจากไม่พบข้อมูลรูปแบบตู้');
        }
    }

    //call page Edit
    function JSvCallPagePosShopEventEdit(ptBchCode,ptPosCode,ptShpCode,pnPshStaUse,pnPshSceLayout,pnPshStaType){
        var tShpTypeCode    = $('#oetPshPSHShpType').val();
        var tShpType        = $('#ocmShpType').val();
        const nPshStaUse    = pnPshStaUse;
        const nPshSceLayout = pnPshSceLayout;
        const nPshStaType = pnPshStaType
        $.ajax({
        type: "POST",
        ...
        success: function(tResult) {
            $('#odvPshContent').html(tResult);
            $("#ocmPshStaType").val(pnPshStaType).selectpicker('refresh');
