...
function JSoAddPosShop(ptRoute) {
    $('#ofmAddPosShop').validate({
        ...
        messages: {
            ...
            ocmPshStaType: $('#ocmPshStaType').data('validate'),
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                success: function(oResult) {
                    ...
                    console.log(aReturn['aClearDataLay'])
                    if (aReturn['nStaEvent'] == 1) {
                        if (aReturn['aClearDataLay'] != '' && aReturn['aClearDataLay']['rtCode'] == 999) {
                            FSvCMNSetMsgWarningDialog('ไม่สามารถแก้ไขชนิดตู้ฝากได้ เพราะมีช่องฝากบางช่องถูกใช้งานอยู่');
                            JCNxCloseLoading();
                        }
        ...
        $('#osmConfirm').on('click', function(evt) {
            $.ajax({
                ...
                success: function(tResult) {
                    ...
                    // ptBchCode = null;
                    if (aResult['nStaEvent'] == 999) {
                        FSvCMNSetMsgWarningDialog('ไม่สามารถลบเครื่องจุดขายได้เพราะมีช่องฝากบางช่องถูกใช้งานอยู่');
                    }
    ...
    if (aDataSplitlength > 1) {
        ...
        $.ajax({
            ...
            success: function(aReturn) {
                var aResult = JSON.parse(aReturn);
