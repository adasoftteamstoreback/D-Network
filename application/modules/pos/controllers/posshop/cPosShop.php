...
class cPosShop extends MX_Controller {
    ...
    public function FSvCPSHListPage(){ 
        ...
        $tBchCodeHD        = $this->input->post('tBchCodeHD');
        ...
        $aData = array(
            ...
            'tBchCodeHD'    => $tBchCodeHD 
        );
        $tRefCode =  json_encode($aData);
        $aChkShopLayout = $this->mPosShop->FSaMPSHChkShopLayout($aData);
        $aAlwEventPosShop	    = FCNaHCheckAlwFunc('salemachine/0/0'); 
        $this->load->view('pos/posshop/wPosShopList', array(
            ...
            'tRefCode'          => $tRefCode,
            'aChkShopLayout'   => $aChkShopLayout['rtCode']  
        ));
    }
    ...
    public function FSoCPSHAddEvent(){  
        try{
            ...
            $tPosCodeSNOld  = $this->input->post('oetPosCodeSNOld');
            ...
            for($i=0; $i<$nCountBranch; $i++){
                $aData   = array(
                    ...
                    'FTPshType'     => $this->input->post('ocmPshStaType'),                  
            ...
            if ($aData['FTPshType'] == 1) {
                $aGetDataPOS = $this->mPosShop->FSaMPSGetDataPOS($aData);
                $aGetDataLay = $this->mPosShop->FSaMPSHGetDataLay($aData);

                if ($aGetDataPOS['rtCode'] == 1 && $aGetDataLay['rtCode'] == 1) {
                    foreach ($aGetDataLay['raItemsLAY'] as $key => $aData) {
                        $aDataGenStaLocker = array(
                            'FTAgnCode'     => $this->session->userdata('tSesUsrAgnCode'),
                            'FTBchCode'     => $aGetDataPOS['raItemsPOS'][0]['FTBchCode'],
                            'FTShpCode'     => $aGetDataPOS['raItemsPOS'][0]['FTShpCode'],
                            'FTPosCode'     => $aGetDataPOS['raItemsPOS'][0]['FTPosCode'],
                            'FTPosCodeOld'  => $tPosCodeSNOld,
                            'FNLayNo'       => $aData['FNLayNo'],
                            'FTLayStaUse'   => 1,
                            'FDCreateOn'    => date('Y-m-d H:i:s'),
                            'FDLastUpdOn'   => date('Y-m-d H:i:s'),
                            'FTCreateBy'    => $this->session->userdata('tSesUsername'),
                            'FTLastUpdBy'   => $this->session->userdata('tSesUsername'),
                            'FTLksToSize'   => $aData['FTPzeCode'],
                        );
                        
                        $this->mPosShop->FSaMPSHGenLockerStatus($aDataGenStaLocker);
                    }
                }
            }
    ...
    public function FSoCPSHEditEvent(){  
        try{
            $tPshType = $this->input->post('ocmPshStaType');
            $aData   = array(
            ...
            $this->db->trans_begin();
            $aClearDataLay = $this->mPosShop->FSaMPSHClearLockerStatus($aData,$tPshType);

            if ($aClearDataLay['rtCode'] == 999) {
                $aData['FTPshType'] = 1;
            }else{
                $aData['FTPshType'] = $this->input->post('ocmPshStaType');
            }
            ...
            if ($aData['FTPshType'] == 1) {
                ...
                if ($aClearDataLay['rtCode'] != 999) {
                    if ($aGetDataPOS['rtCode'] == 1 && $aGetDataLay['rtCode'] == 1) {
                        foreach ($aGetDataLay['raItemsLAY'] as $key => $aDataLay) {
                            $aDataGenStaLocker = array(
                                'FTAgnCode'     => $this->session->userdata('tSesUsrAgnCode'),
                                'FTBchCode'     => $aGetDataPOS['raItemsPOS'][0]['FTBchCode'],
                                'FTShpCode'     => $aGetDataPOS['raItemsPOS'][0]['FTShpCode'],
                                'FTPosCode'     => $aGetDataPOS['raItemsPOS'][0]['FTPosCode'],
                                'FTPosCodeOld'  => $aData['FTPosCodeOld'],
                                'FNLayNo'       => $aDataLay['FNLayNo'],
                                'FTLayStaUse'   => 1,
                                'FDCreateOn'    => date('Y-m-d H:i:s'),
                                'FDLastUpdOn'   => date('Y-m-d H:i:s'),
                                'FTCreateBy'    => $this->session->userdata('tSesUsername'),
                                'FTLastUpdBy'   => $this->session->userdata('tSesUsername'),
                                'FTLksToSize'   => $aDataLay['FTPzeCode'],
                            );
                            $this->mPosShop->FSaMPSHGenLockerStatus($aDataGenStaLocker);
                        }
                    }
    ...
    public function FSoCPSHDeleteEvent(){ 
        ...
        $aClearDataLay = $this->mPosShop->FSaMPSHClearLockerStatus($aDataMaster,2);
