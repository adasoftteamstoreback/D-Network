...
class mSaleMachine extends CI_Model
{
    ...
    public function FSaMPOSList($paData){
        try {
            ...
            $tSQL   = "
                SELECT c.* FROM(
                    SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPosCode DESC) AS rtRowID,* 
                    FROM (
                        SELECT DISTINCT
                            SAL.FTPosCode   AS rtPosCodeLef,
                        ...
                        LEFT JOIN [TCNMBranch_L]    BCHL    WITH(NOLOCK) ON POS.FTBchCode = BCHL.FTBchCode AND BCHL.FNLngID = ".$this->db->escape($tLngID)."
                        LEFT JOIN TVDMPosShop       PSHP    WITH(NOLOCK) ON POS.FTBchCode = PSHP.FTBchCode AND POS.FTPosCode = PSHP.FTPosCode
                        LEFT JOIN TRTMShopPos       RTSP    WITH(NOLOCK) ON POS.FTBchCode = RTSP.FTBchCode AND POS.FTPosCode = RTSP.FTPosCode AND RTSP.FTShpCode = POS.FTPosCode
                        LEFT JOIN TCNMShop_L        SHPL    WITH(NOLOCK) ON PSHP.FTShpCode = SHPL.FTShpCode AND PSHP.FTBchCode = SHPL.FTBchCode AND SHPL.FNLngID    = ".$this->db->escape($tLngID)."
                        ...
                        LEFT JOIN [TCNMWaHouse_L]   WAHL    WITH(NOLOCK) ON POS.FTPosCode = WAHL.FTWahCode AND POS.FTBchCode = WAHL.FTBchCode AND WAHL.FNLngID      = ".$this->db->escape($tLngID)."
                        LEFT JOIN [TCNMPos_L]       POS_L   WITH(NOLOCK) ON POS_L.FTPosCode = POS.FTPosCode AND POS.FTBchCode = POS_L.FTBchCode AND POS_L.FNLngID   = ".$this->db->escape($tLngID)."
                        LEFT JOIN [TCNMChannel_L]   CHN_L   WITH(NOLOCK) ON POS.FTChnCode = CHN_L.FTChnCode  AND CHN_L.FNLngID = ".$this->db->escape($tLngID)."
                        LEFT JOIN (SELECT DISTINCT FTPosCode FROM TRTTSalHD WITH(NOLOCK)) SAL ON POS.FTPosCode = SAL.FTPosCode
            ...
            if (isset($tSearchList) && !empty($tSearchList)) {
                $tSQL   .= " AND (POS.FTPosCode  LIKE '%".$this->db->escape_like_str($tSearchList)."%'";
                $tSQL   .= " OR BCHL.FTBchName   LIKE '%".$this->db->escape_like_str($tSearchList)."%' ";
                $tSQL   .= " OR CHN_L.FTChnName  LIKE '%".$this->db->escape_like_str($tSearchList)."%' ";
                $tSQL   .= " OR POS_L.FTPosName  LIKE '%".$this->db->escape_like_str($tSearchList)."%')";
            }
    ...
    public function FSoMPOSGetPageAll($ptSearchList){
        try {
            ...
            $tSQL   = "
                SELECT COUNT (rtRowID) AS counts FROM(
                    SELECT  ROW_NUMBER() OVER(ORDER BY FDCreateOn DESC , rtPosCode DESC) AS rtRowID,*
                    FROM (
                        SELECT DISTINCT
                            POS.FTPosCode   AS rtPosCode,
            ...
            if (isset($ptSearchList) && !empty($ptSearchList)) {
                $tSQL   .= " AND (POS.FTPosCode LIKE '%".$this->db->escape_like_str($ptSearchList)."%'";
                $tSQL   .= " OR BCHL.FTBchName  LIKE '%".$this->db->escape_like_str($ptSearchList)."%' ";
                $tSQL   .= " OR CHN_L.FTChnName LIKE '%".$this->db->escape_like_str($ptSearchList)."%' ";
                $tSQL   .= " OR POS_L.FTPosName LIKE '%".$this->db->escape_like_str($ptSearchList)."%')";
            }
            $tSQL .= ") Base) AS c ";
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
        ...
        try {
            ...
            $tSQL   = " 
                SELECT
                    ...
                    POS.FTPosStaVatSend  AS  rtPosStaVatSend,
                    POS.FTPosStaUse  AS  rtPosStaUse,
                    POS.FTPosStaFranchise  AS  rtPosStaFranchise,
                ...
                LEFT JOIN [TCNMWaHouse_L] POSWH_L WITH(NOLOCK) ON  POSWH.FTWahCode = POSWH_L.FTWahCode AND POS.FTBchCode = POSWH_L.FTBchCode AND POSWH_L.FNLngID = ".$this->db->escape($tLang)."
                LEFT JOIN [TCNMBranch_L] BCH_L WITH(NOLOCK) ON  POS.FTBchCode = BCH_L.FTBchCode  AND BCH_L.FNLngID = ".$this->db->escape($tLang)."
                LEFT JOIN [TCNMPos_L] POS_L WITH(NOLOCK)  ON POS_L.FTPosCode = POS.FTPosCode AND POS.FTBchCode = POS_L.FTBchCode AND POS_L.FNLngID = ".$this->db->escape($tLang)."
                LEFT JOIN TCNMSlipMsgHD_L SMP_L WITH(NOLOCK) ON POS.FTSmgCode = SMP_L.FTSmgCode AND SMP_L.FNLngID = ".$this->db->escape($tLang)."
                LEFT JOIN TCNMChannel_L CHN_L ON POS.FTChnCode = CHN_L.FTChnCode  AND CHN_L.FNLngID = ".$this->db->escape($tLang)."
                WHERE POS.FDCreateOn <> ''
                AND POS.FTPosCode   = ".$this->db->escape($tPosCode)."
                AND POS.FTBchCode   = ".$this->db->escape($tBchCode)."
            ";
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
    ...
    {
        $tSQL   = "
            SELECT COUNT(POS.FTPosCode) AS counts
            FROM TCNMPos POS
            WHERE POS.FTPosCode = ".$this->db->escape($ptPosCode)."
            AND POS.FTBchCOde   = ".$this->db->escape($pBchCode)."
        ";
        $oQuery = $this->db->query($tSQL);
        if ($oQuery->num_rows() > 0) {
    ...
    public function FSaMPOSAddUpdateMaster($paDataSaleMachine)
    {
        try {
            ...
            $this->db->update('TCNMPos', array(
                ...
                'FTPosIP'           => $paDataSaleMachine['FTPosIP'],
    ...
    public function FSxMPOSUpdatePos($paDataPos){
        ...
        if ($paDataPos["ocbPosStaSaleFC"] != null) {
            $tSQL .= " FTPosStaFranchise = '" . $paDataPos["ocbPosStaSaleFC"] . "',";
        } else {
            $tSQL .= " FTPosStaFranchise = '2',";
        }
    ...
    public function FSxMPOSInsertPos($paDataPos)
    {
        ...
        $tSQL = "INSERT INTO TCNMPos (
                    FTBchCode,FTPosCode,FTPosType,FTPosRegNo,FTPosIP,FTSmgCode,
                    FNPosShwRow,FNPosShwCol,
                    FTPosStaPrnEJ,FTPosStaVatSend,FTPosStaUse,FTPosStaFranchise,FTPosStaSumScan,FTPosStaSumPrn,FTPosStaShift,
        ...
        if ($paDataPos["ocbPosStaSaleFC"] != null) {
            $tSQL .= "'" . $paDataPos["ocbPosStaSaleFC"] . "',";
        } else {
            $tSQL .= "'2',";
        }
    ...
    public function FSaMSMGGetDataList($tPosCode){
        try {
            ...
            $tSQL   = "
                ...
                FROM TCNMPos
                LEFT JOIN TCNMSlipMsgHD_L ON TCNMPos.FTSmgCode = TCNMSlipMsgHD_L.FTSmgCode AND TCNMSlipMsgHD_L.FNLngID = ".$this->db->escape($nLngID)."
                WHERE TCNMPos.FTPosCode = ".$this->db->escape($tPosCode)."
            ";
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
    ...
    public function FSaMSMGGetVDPosShopDataList($tPosCode)
    {
        try {
            $tSQL   = "
                ...
                WHERE TPS.FDCreateOn <> ''
                AND TPS.FTPosCode = ".$this->db->escape($tPosCode)."
            ";
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
                $aList = $oQuery->row_array();
    ...
    public function FSaMImportGetDataInTmp(array $paParams = []){
        ...
        $tSQL   = "
            ...
            WHERE 1=1
            AND TMP.FTSessionID     = ".$this->db->escape($tUserSessionID)."
            AND TMP.FTTmpTableKey   = ".$this->db->escape($tTableKey)."
        ";

        if ($tTextSearch != '' || $tTextSearch != null) {
            $tSQL .= "
                AND (
                    TMP.FTBchCode       LIKE '%".$this->db->escape_like_str($tTextSearch)."%'
                    /*OR BCHL.FTBchName LIKE '%".$this->db->escape_like_str($tTextSearch)."%'*/
                    OR TMP.FTPosCode    LIKE '%".$this->db->escape_like_str($tTextSearch)."%'
                    OR TMP.FTPosName    LIKE '%".$this->db->escape_like_str($tTextSearch)."%'
                )
            ";
        }
        ...
        if ($tTypeCaseDuplicate == "2") { // อัพเดทรายการเดิม
            ...
            $tSQLUpdate = "
                ...
                SET
                    ...
                    TCNMPos.FTPosIP = TCNTImpMasTmp.FTPosIP,
        ...
        $tSQL = "
            SELECT TOP 1
                (
                    ...
                    WHERE IMP.FTSessionID   = ".$this->db->escape($tUserSessionID)."
