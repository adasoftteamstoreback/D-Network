...
class mPosShop extends CI_Model {
    ...
    public function FSaMPSGetDataPOS($paData)
    {
        $tPosCode   = $paData['FTPosCode'];
        $tShpCode   = $paData['FTShpCode'];
        $tBchCode   = $paData['FTBchCode'];
    ...
    public function FSaMPSHGetDataLay($paData)
    {
        $tPosCode   = $paData['FTPosCode'];
        $tShpCode   = $paData['FTShpCode'];
        $tBchCode   = $paData['FTBchCode'];
    ...
    public function FSaMPSHGenLockerStatus($paDataGenStaLocker){
        try{
            $this->db->insert('TRTTLockerStatus', array(
                'FTAgnCode'     => $paDataGenStaLocker['FTAgnCode'],
                'FTBchCode'     => $paDataGenStaLocker['FTBchCode'],
                'FTShpCode'     => $paDataGenStaLocker['FTShpCode'],
                'FTPosCode'     => $paDataGenStaLocker['FTPosCode'],
                'FNLayNo'       => $paDataGenStaLocker['FNLayNo'],
                'FTLayStaUse'   => $paDataGenStaLocker['FTLayStaUse'],
                'FDCreateOn'    => $paDataGenStaLocker['FDCreateOn'],
                'FDLastUpdOn'   => $paDataGenStaLocker['FDLastUpdOn'],
                'FTCreateBy'    => 'System',
                'FTLastUpdBy'   => $paDataGenStaLocker['FTLastUpdBy'],
                'FTLksToSize'   => $paDataGenStaLocker['FTLksToSize'],
            ));
            if($this->db->affected_rows() > 0){
                $aStatus = array(
                    'rtCode' => '1',
    ...
    public function FSaMPSHClearLockerStatus($paDataGenStaLocker,$ptPshType){
        try{
            $tPosCode    = $paDataGenStaLocker['FTPosCode'];
            $tPosCodeOld = $paDataGenStaLocker['FTPosCodeOld'];
            $tBchCode    = $paDataGenStaLocker['FTBchCode'];
            $tShpCode    = $paDataGenStaLocker['FTShpCode'];
            
            if ($ptPshType == 2) {
                if ($paDataGenStaLocker['FTPosCodeOld'] != '') {
                    $tSQLChk = "SELECT * FROM TRTTLockerStatus
                                    WHERE (FTLayStaUse = 2 OR  FTLayStaUse = 4)
                                    AND FTPosCode IN ('$tPosCodeOld') 
                                    AND FTShpCode = '$tShpCode'
                                    AND FTBchCode = '$tBchCode' ";
                    $oQuery = $this->db->query($tSQLChk);
                    if ($oQuery->num_rows() > 0){
                        $aStatus = array(
                            'rtCode' => '999',
                            'rtDesc' => 'ลบไม่สำเร็จมีช่องที่ถูกใช้ไปแล้ว',
                        );
                    }else{
                        $tSQL = "DELETE FROM TRTTLockerStatus
                            WHERE 1=1 
                            AND FTPosCode IN ('$tPosCodeOld') 
                            AND FTShpCode = '$tShpCode'
                            AND FTBchCode = '$tBchCode' ";
                        ...
                        if($this->db->affected_rows() > 0){
                            $aStatus = array(
                                'rtCode' => '1',
                                'rtDesc' => 'Clear LockerStatus Success',
                            );
                        }else{
                            $aStatus = array(
                                'rtCode' => '905',
                                'rtDesc' => 'Error Cannot Clear LockerStatus',
                            );
                        }
                    }
                }else{
    ...
    public function FSaMPSHChkShopLayout($paData)
    {
        $tBchCode    = $paData['tBchCodeHD'];
        $tShpCode    = $paData['tPSHShpCode'];

        $tSQLChk = "SELECT TOP 1 * 
                        FROM TRTMShopLayout
                    WHERE FTShpCode = '$tShpCode'
                    AND FTBchCode = '$tBchCode' ";
        $oQuery = $this->db->query($tSQLChk);
        if ($oQuery->num_rows() > 0){
            $aStatus = array(
