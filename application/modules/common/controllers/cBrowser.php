...
class cBrowser extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($oPtions != '' || $oPtions != 'undefined') :
            ...
            $CallBackType   = (isset($oPtions['CallBack']['ReturnType'])) ? $oPtions['CallBack']['ReturnType'] : "S";
            $tSearchFilter  = (isset($oPtions['SearchFilter'])) ? $oPtions['SearchFilter'] : '';
            $nShowRecord    = get_cookie('nShowRecordInPageList');
            ...
            if ($aDistinct != '') {
                ...
                $tSQL .= "SELECT TOP $nShowRecord  $tResultShow FROM ( SELECT ";
            } else {
                $tSQL .= "SELECT TOP $nShowRecord  ";
            }
            ...
            if (isset($tFilerGride)) {
                if ($tFilerGride != '') {
                    ...
                    for ($fc = 0; $fc < FCNnHSizeOf($oPtions['GrideView']['DataColumns']); $fc++) {
                        ...
                        $bCaseStop = true;
                        if (strpos($tFilterCol, ".") !== false) {// มี การใส่ AS มา Ex : Table.Column
                            $tTextAsTable = explode(".",$tFilterCol);
                            switch ($tTextAsTable[0]) {
                                case "T1":  // |       
                                case "T2":  // |
                                case "T3":  // |
                                case "T4":  // > ถ้าเป็นพวกข้อมูลรถ (fitauto) เค้าใส่ AS มา ไม่ต้องไปค้นหา
                                case "T5":  // |
                                case "T6":  // |
                                case "T7":  // |
                                case "T8":  // |
                        ...
                        //จะไม่ต้อง ค้นหา PK ที่ซ้ำกับข้างบน
                        if($tFilterCol != $tColPk){
                            if (isset($tFilterCol) && !empty($tFilterCol) && (strpos($tFilterCol,'FN')==0 || strpos($tFilterCol,'FD')==0)) {	
								if (isset($oPtions['GrideView']['DisabledColumns'])){
									//ตัวที่ disabled อยู่ไม่ต้องเอามาค้นหา
									if($this->JCNtColDisabled($fc, $oPtions['GrideView']['DisabledColumns']) == false){
										if($bCaseStop == true){
                ...
                if (isset($oPtions['Where'])) :
                    if ($oPtions['Where']['Condition']) :
                        for ($w = 0; $w < FCNnHSizeOf($oPtions['Where']['Condition']); $w++) {
                            ...
                            $tReplaceWhere = str_replace("&lt;","<",$oPtions['Where']['Condition'][$w]);
                            $tSQL .= " " . $tReplaceWhere;
                        }
                    endif;
                endif;
            ...
            if (isset($oPtions['BrowseLev'])) {
                if ($oPtions['BrowseLev'] != 1) {
                    ...
                    //เช็คสิทธิก่อนจะมีปุมเพิ่ม
                    $aAlwPermission = FCNaHCheckAlwFunc($tFormAddRouteName.'/0/0');
                    if($aAlwPermission['tAutStaAdd'] == 1 ){ //ถ้ามีสิทธิเพิ่มข้อมูล
                        $tDataTable .= "
                ...
                foreach ($aDataTable as $key => $val) :
                    ...
                    if ($tIDCurrent != $val->$tMasterPK) :
                        ...
                        elseif ($CallBackType == 'M') :
                            ...
                            $tDataTable .= '<tr class="xCNTextDetail2 ' . $tRowActive . ' " onclick="JCNxPushMultiSelection(' . "'" . $val->$tMasterPK . "','" . str_replace('"', '',$tCallBackTextData) . "',this" . ')">';
                        else :
                        ...
                        for ($f = 0; $f < FCNnHSizeOf($oPtions['GrideView']['DataColumns']); $f++) :
                            ...
                            if (isset($oPtions['GrideView']['DisabledColumns'])) {
                                ...
                                } else if($oPtions['Table']['Master'] == 'TCNMPrinter' && $oPtions['Table']['PK'] == 'FTPrnCode'){
                                    //สำหรับข้อมูลตารางปริ้นเตอร์ TCNMPrinter
                                    if ($tColumnVal == 'FTPrnType' && $tDataDisPlay == 1 ) {
                                        $tColumnValue = 'บรรทัด / บรรทัด';
                                    }else if ($tColumnVal == 'FTPrnType' && $tDataDisPlay == 2 ) {
                                        $tColumnValue = 'รวมพิมพ์โดยตรง';
                                    }else if ($tColumnVal == 'FTPrnType' && $tDataDisPlay == 3 ) {
                                        $tColumnValue = 'รวมพิมพ์โดยวินโดว์';
                                    }else if ($tColumnVal == 'FTPrnType' && $tDataDisPlay == 4 ) {
                                        $tColumnValue = 'รวมพิมพ์โดยรายงาน';
                                    }else if ($tColumnVal == 'FTPrnType' && $tDataDisPlay == 0 ) {
                                        $tColumnValue = 'ไม่ได้ระบุ';
                                    }else{
                                        $tColumnValue = $this->JCNtColChkNull($tDataDisPlay);
                                    }
                                }else {
            ...
            $tDataTable .= "<div class='col-xs-12 col-sm-6 col-md-6 col-lg-6'>";
            $tDataTable .= "<p>แสดงข้อมูลรายการล่าสุด " . $nShowRecord . " รายการ</p>";
            $tDataTable .= "</div>";
