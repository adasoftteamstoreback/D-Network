...
class mCustomer extends CI_Model {
    ...
    public function FSaMCSTSearchByID($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tCreditSQL  =  "SELECT DISTINCT
            ...
            LEFT JOIN [TCNMShipVia_L] VIAL  WITH(NOLOCK) ON VIAL.FTViaCode = CRD.FTViaCode AND VIAL.FNLngID = $nLngID
    ...
    public function FSaMCSTList($ptAPIReq, $ptMethodReq, $paData){
        ...
        $tSQL     = " SELECT TOP ". get_cookie('nShowRecordInPageList')." c.* FROM(
                    ...
                    *
                        ...
                        LEFT JOIN [TCNMCst_L]  CSTL WITH(NOLOCK) ON CST.FTCstCode = CSTL.FTCstCode AND CSTL.FNLngID = $nLngID
                        LEFT JOIN [TCNMCstGrp_L] CSTGL WITH(NOLOCK) ON CSTGL.FTCgpCode = CST.FTCgpCode AND CSTGL.FNLngID = $nLngID
                        LEFT JOIN [TCNMImgPerson] IMGP WITH(NOLOCK) ON IMGP.FTImgRefID = CST.FTCstCode
                        LEFT JOIN [TCNMCstLev] CSTLev WITH(NOLOCK) ON CSTLev.FTClvCode = CST.FTClvCode
                        LEFT JOIN [TCNMCstLev_L] CSTLevL WITH(NOLOCK) ON CSTLevL.FTClvCode = CSTLev.FTClvCode AND CSTLevL.FNLngID = $nLngID
                        LEFT JOIN [TCNMPdtPriList_L] PRIL WITH(NOLOCK) ON CSTLev.FTPplCode = PRIL.FTPplCode AND PRIL.FNLngID = $nLngID
                        WHERE 1=1";
    ...
    public function FSaMCSTAddUpdateMaster($paData){
        ...
        try{
            ...
            // $this->db->set('FTPmgCode', $paData['FTPmgCode']); //ถอดออกจาก Master Customer 23/06/2021
            ...
            if($this->db->affected_rows() > 0){
                ...
                $this->db->insert('TCNMCst',array(
                    ...
                    // 'FTPmgCode'             => $paData['FTPmgCode'], //ถอดออกจาก Master Customer 23/06/2021
  ...
  /**
	...
	public function FSxMCSTFindCstCredit() {
		$tRoleCode 	= $this->session->userdata("tSesUsrRoleCodeMulti");
		$tSQL 		= "SELECT TOP 1 FTUfrStaAlw FROM TCNTUsrFuncRpt AS RPT WHERE RPT.FTRolCode IN ($tRoleCode) AND FTUfrGrpRef = '086' AND FTGhdApp = 'SB' ";
		$oQuery 	= $this->db->query($tSQL);
		if ($oQuery->num_rows () > 0) {
			return 1;
		} else {
    ...
    public function FSxMCSTFindCstCreditCode(){
		$tSQL 		= "SELECT TOP 1 ISNULL(FTSysStaUsrValue,'') AS FTSysStaUsrValue FROM TSysConfig WHERE FTSysCode = 'tPS_CstType' AND FTSysSeq = '2'";
		$oQuery 	= $this->db->query($tSQL);
		if ($oQuery->num_rows () > 0) {
			$oDetail = $oQuery->row_array();
            $aResult = array(
