...
function JSxCheckCustomerValidateForm() {
    ...
    $('#ofmAddCustomerInfo1').validate({
        ...
        messages: {
            ...
            oetCstTel: {
                "required": $('#oetCstTel').attr('data-validate-required')
 ...
 * Return Type : -
 */
function JSxCSTVisibledDelAllBtn() { // Action start after change check box value.
    try {
        var nCheckedCount = $('#odvRGPList td input:checked').length;
        if (nCheckedCount > 1) {
...
function JSxCSTVisibledActionSubMenu(poElement, poEvent) {
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function(poElement) {
        ...
        //ลูกค้าเครดิต
        if ($('#ohdNavActive').val() == 'oliCstCredit') {
            $('#obtSubCancel').addClass('hidden');
        }
    });
