...
class mSmartlockerSize extends CI_Model {
        ...
        try{
            ...
            $tSQL       = "SELECT c.* FROM(
                ...
                if(isset($tSearchList) && !empty($tSearchList)){
                    $tSQL .= " AND (SHPS.FTPzeCode LIKE '%$tSearchList%' OR SHPSL.FTSizName LIKE '%$tSearchList%')";
                }
    ...
    public function FSoMSMLGetPageAll($ptSearchList,$paData){
        try{
            ...
            $tSQL = "SELECT COUNT (SMS.FTPzeCode) AS counts
                     ...
                     LEFT JOIN [TRTMShopSize_L]  SHPSL ON SMS.FTPzeCode = SHPSL.FTSizCode AND SHPSL.FNLngID = $tFNLngID  
                     WHERE 1=1 ";

            if(isset($ptSearchList) && !empty($ptSearchList)){
                $tSQL .= " AND (SMS.FTPzeCode LIKE '%$ptSearchList%' OR SHPSL.FTSizName LIKE '%$ptSearchList%')";
            }
            $oQuery = $this->db->query($tSQL);
            if ($oQuery->num_rows() > 0) {
