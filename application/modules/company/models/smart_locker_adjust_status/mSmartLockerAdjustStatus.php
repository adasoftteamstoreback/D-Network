...
class mSmartLockerAdjustStatus extends CI_Model {
    ...
    public function FSaMSMLKAdjStaRackChannelList($paParams){
        ...
        $tSQL = "   SELECT c.* FROM(
                        ...
                        FROM
                        (
                        SELECT LST.FNLayNo,LAY.FTLayName,LST.FTLayStaUse,LST.FTBchCode,SHPLAY.FNLayRow,
	SHPLAY.FNLayCol,LST.FTShpCode
                        FROM TRTTLockerStatus LST
                        LEFT JOIN TRTMShopLayout SHPLAY WITH ( NOLOCK ) ON LST.FNLayNo = SHPLAY.FNLayNo 
	                    AND SHPLAY.FTBchCode IN ( $tBchCode ) 
	                    AND SHPLAY.FTShpCode = '$tShpCode'
                        INNER JOIN TRTMShopLayout_L LAY ON LST.FTBchCode = LAY.FTBchCode
                                                            AND LST.FTShpCode = LAY.FTShpCode
                                                            AND LST.FNLayNo = LAY.FNLayNo
                                                            AND LAY.FNLngID = 1
                        WHERE LST.FTBchCode = $tBchCode
                        AND LST.FTShpCode = '$tShpCode'
                        AND LST.FTPosCode = '$tPosCode'
                ";
    ...
    public function FSaMSMLKAdjStaTempList($paParams){
        ...
        $tSQL = "   SELECT c.* FROM(
                        ...
                        LEFT JOIN TRTMShopLayout_L SLYL WITH (NOLOCK) ON TMP.FNLayNo = SLYL.FNLayNo AND TMP.FTShpCode = SLYL.FTShpCode AND TMP.FTBchCode = SLYL.FTBchCode AND SLYL.FNLngID = $nLngID
                        WHERE TMP.FTBchCode IN ($tBchCode)
                        AND TMP.FTShpCode = '$tShpCode'
                        AND TMP.FTMttSessionID = '$tSessionID'  
