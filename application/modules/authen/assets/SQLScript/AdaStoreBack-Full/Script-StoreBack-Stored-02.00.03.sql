    ...
    SET @tSQL += '	DT.FTPdtCode,
										...
										CASE
												WHEN HD.FNXshDocType = ''1'' THEN
														ISNULL(FCXsdSetPrice, 0) * ISNULL(FCXsdQty, 0)
												ELSE(
														ISNULL(FCXsdSetPrice, 0) * ISNULL(FCXsdQty, 0)
												) *- 1
												END AS FCXsdAmt,
										CASE
												WHEN HD.FNXshDocType = ''1'' THEN
														ISNULL(FCXddDTDis, 0) + ISNULL(FCXddDTDisPmt, 0)
												ELSE(
														ISNULL(FCXddDTDis, 0) + ISNULL(FCXddDTDisPmt, 0)
												) *- 1
												END AS FCXsdDis,
										CASE
												WHEN HD.FNXshDocType = ''1'' THEN
														ISNULL(FCXsdNet, 0) + ISNULL(FCXddDTDisPmt, 0)
												ELSE
														( ISNULL(FCXsdNet, 0) + ISNULL(FCXddDTDisPmt, 0) ) *- 1
												END AS FCXsdNet ,
										CASE
												WHEN HD.FNXshDocType = ''1'' THEN
														ISNULL(FCXsdQty, 0)
										...
										Pdt.FCPdtCostStd AS FCXshCost ,

										CASE
												WHEN Pdt.FCPdtCostStd = ''0'' THEN
														0
												ELSE
														Pdt.FCPdtCostStd + (Pdt.FCPdtCostStd * (DT.FCXsdVatRate / 100))
										...
										CASE
												WHEN Pdt.FCPdtCostStd = ''0'' THEN
														0
												ELSE
														Pdt.FCPdtCostStd + (Pdt.FCPdtCostStd * (DT.FCXsdVatRate / 100)) * ISNULL(FCXsdQty, 1)
												END AS FCXshCostTotal,

										(ISNULL(FCXsdSetPrice, 0) * ISNULL(FCXsdQty, 0)) - (Pdt.FCPdtCostStd + (Pdt.FCPdtCostStd * (DT.FCXsdVatRate / 100)) * ISNULL(FCXsdQty, 1)) AS FCXshProfit,

										CASE
												WHEN Pdt.FCPdtCostStd = ''0'' THEN
														100
												ELSE
														(((ISNULL(FCXsdSetPrice, 0) * ISNULL(FCXsdQty, 0)) - (Pdt.FCPdtCostStd + (Pdt.FCPdtCostStd * (DT.FCXsdVatRate / 100)) * ISNULL(FCXsdQty, 1))) * 100) / Pdt.FCPdtCostStd
    ...
    SET @tSQL += ' LEFT JOIN (
                                    ...
                                    LEFT JOIN (
                                        ...
                                        GROUP BY
                                            ...
                                            FTXshDocNo,
                                            FNXsdSeqNo
                                    ) DTDisPmt ON DT.FTBchCode = DTDisPmt.FTBchCode AND DT.FTXshDocNo = DTDisPmt.FTXshDocNo AND DT.FNXsdSeqNo = DTDisPmt.FNXsdSeqNo 
                                    LEFT JOIN (
                                            SELECT
                                                    HD.FTBchCode,
                                                    HD.FTXshDocNo,
...
BEGIN
    ...
    IF @tPriceType = 'Price4Cst' BEGIN
        ...
        SET @tSQL += 'LEFT JOIN ( '
			...
			SET @tSQL += 'FROM ( '
			SET @tSQL += '	SELECT '
			SET @tSQL += '		ROW_NUMBER() OVER (PARTITION BY FTPdtCode,FTPunCode ORDER BY FTPdtCode ASC, FDPghDStart DESC) AS FNRowPart, '
