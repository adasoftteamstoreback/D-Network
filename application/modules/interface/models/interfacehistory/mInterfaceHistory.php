...
class mInterfaceHistory extends CI_Model
{
    ...
    public function FSaMIFHList($paData)
    {
        ...
        $tSubQuery = "  
            SELECT  
                ...
                LOG6.FTLogRefNo
            FROM TLKTLogHis HIS WITH (NOLOCK)
            LEFT JOIN TLKTLogAPI6 LOG6 WITH (NOLOCK) ON HIS.FTlogTaskRef = LOG6.FTXshDocNo
        ...
        if (!empty($tSearchList)) {
            $tSubQuery .= " AND HIS.FTLogTask LIKE '%$tSearchList%'";
        }
        ...
        if (!empty($tIFHDateFrom) && !empty($tIFHDateTo)) {
            $tSubQuery .= " AND ((HIS.FDLogCreate BETWEEN CONVERT(datetime,'$tIFHDateFrom 00:00:00') AND CONVERT(datetime,'$tIFHDateTo 23:59:59')) ";
            $tSubQuery .= " OR (HIS.FDLogCreate BETWEEN CONVERT(datetime,'$tIFHDateTo 23:00:00') AND CONVERT(datetime,'$tIFHDateFrom 00:00:00'))) ";
        }
        ...
        $tSQL = " 
            ...
            FROM(
                SELECT  
                    ROW_NUMBER() OVER(ORDER BY FDLastUpdOn DESC) AS FNRowID,* 
                FROM (
                    $tSubQuery
    ...
    //path ส่งออก
    public function FStMIFHGetPathLoadFileOutbound()
    {
        $tSQL = "   
            SELECT TOP 1
                CASE 
