...
// Functionality:ตรวจสอบว่า Role ต่ำกว่าหรือไม่
// Parameters: Array เงื่อนไขการเช็คค่า [paParams]
// Creator: 15/09/2021 Off
// Last Modified: -
// Return: Status
// Return Type: Number
// 
function FCNnMasTmpChkRoleInDB(array $paParams = [])
{
    if(
        !isset($paParams['tUserSessionID']) || empty($paParams['tUserSessionID']) 
        || !isset($paParams['tUsrRoleSpc']) || empty($paParams['tUsrRoleSpc']) 
        || !isset($paParams['tFieldName']) || empty($paParams['tFieldName']) 
    ) {
        return;
    }
    ...
    $tUserSessionID = $paParams['tUserSessionID'];
    $tFieldName = $paParams['tFieldName'];
    $tUsrRoleSpc = $paParams['tUsrRoleSpc'];
...
function FCNnMasTmpChkCodeDupInDBSpecial(array $paParams = []){
    ...
    switch ($tTableCheck) {
        ...
        case "TCNMPdt":
            ...
            //                 MAS.FTPdtCode ,
            //                 MAS.FTPunCode , 
            //                 MAS.FTAgnCode AS MAS,
            //                 SPC.FTAgnCode AS SPC,
            //                 PDT.FTPdtCode AS PDT
            //             FROM TCNTImpMasTmp MAS WITH(NOLOCK)
            //             LEFT JOIN TCNMPdtSpcBch SPC WITH(NOLOCK) ON MAS.FTPdtCode = SPC.FTPdtCode 
            //             LEFT JOIN TCNMPdt PDT WITH(NOLOCK) ON MAS.FTPdtCode = PDT.FTPdtCode
            //             WHERE MAS.FTTmpTableKey = 'TCNMPdt' 
            //             AND MAS.FTTmpStatus = 1
            //             AND MAS.FTSessionID = '$tUserSessionID'
            //             AND (MAS.FTAgnCode != SPC.FTAgnCode OR MAS.FTPdtCode = PDT.FTPdtCode)
        ...
        case "TCNMPdt_AD":
            $tSQL = "UPDATE T SET 
                        T.FTTmpStatus = '6', 
                        T.FTTmpRemark = 'มีข้อมูลนี้อยู่แล้วในระบบ'
                    FROM TCNTImpMasTmp T 
                    INNER JOIN (
                        SELECT 
                                MAS.FTSessionID ,
