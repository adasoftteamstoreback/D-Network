...
function FCNoHDOCDeleteDocTmp(){
    ...
    // สินค้า
    $ci->db->where('FDCreateOn <', $tDateNow);
    $ci->db->delete('TCNMPdtPackSizeTmp');

    $ci->db->where('FDCreateOn <', $tDateNow);
    $ci->db->delete('TCNMPdtBarTmp');


    $ci->db->where('FDCreateOn <', $tDateNow);
    $ci->db->delete('TCNMPdtSplTmp');

    //ใบเคลมสินค้า 
    $ci->db->where('FDCreateOn <', $tDateNow);
    $ci->db->delete('TCNTPdtClaimDTTmp');

    //ลบข้อมูลของตาราง Temp ใหม่
    $tTmpDate         = date("d");
    for($i=1; $i<$tTmpDate; $i++){
        $tDateForTmp      = str_pad($i, 2, "0", STR_PAD_LEFT) . date("mY");

        //ลบข้อมูลสินค้าชุด
        $tTableTmp_PDTSet = 'TSVTPdtSet_Tmp' . $tDateForTmp;
        if($ci->db->table_exists($tTableTmp_PDTSet)){
            $tSQL = 'DROP TABLE ' . $tTableTmp_PDTSet;
            $ci->db->query($tSQL);
        }

        //ลบข้อมูลสินค้าชุดบำรุง
        $tTableTmp_PDTSet = 'TSVTPdtSetChk_Tmp' . $tDateForTmp;
        if($ci->db->table_exists($tTableTmp_PDTSet)){
            $tSQL = 'DROP TABLE ' . $tTableTmp_PDTSet;
            $ci->db->query($tSQL);
        }
...
function FCNConvert($num)
{
    ...
    function FCNaDOCEndOfBillCal($paParams){
        ...
        $aResult['tDisChgType'] = $aHDDisTemp['FTXtdDisChgType']; // DisChg HD Text
